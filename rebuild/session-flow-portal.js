...
(function(root,factory){
  ...
  class IntakeFlow{
    constructor({portal}={}){this.client=portal.client('session-intake-flow')}
    beginFromSlots(slotIds,{inputMode='text',title='',note=''}={}){
      ...
      }catch(error){this.client.command('session-schedule','retireOccurrence',{args:[occurrence.id,{note:`Intake begin rollback: ${text(error.message)}`}]});throw error}
    }
    ...
    discardDraft(draftId,{retireOccurrence=false,note=''}={}){const draft=this.client.query('session-lifecycle','getDraft',{args:[draftId]});if(!draft)return false;const removed=this.client.command('session-lifecycle','discardDraft',{args:[draftId]});if(retireOccurrence&&draft.slotKey)this.client.command('session-schedule','retireOccurrence',{args:[draft.slotKey,{note}]});return removed}
    previewDraft(draftId){const draft=this.client.query('session-lifecycle','getDraft',{args:[draftId]});if(!draft)throw new Error(`Draft not found: ${draftId}`);const session=this.client.query('session-truth','parse',{source:draft.source,identity:draft.identity}),validation=this.client.query('session-truth','validate',{session});return{draft,session,validation}}
    acceptDraft(draftId,{select=true}={}){
      ...
      try{const bound=this.client.command('session-schedule','bindSession',{args:[occurrence.id,record.id,{note:'Accepted canonical session'}]},{sessionId:record.id});return{record,bound,session:clone(record.current)}}catch(error){this.client.command('session-lifecycle','markSuperseded',{args:[record.id,{note:`Schedule bind failed: ${text(error.message)}`}]},{sessionId:record.id});throw error}
    }
  }
  function create({scheduleStorage,lifecycleStorage,calendarSources=[],evidenceSources=[],evidenceAliases=[],clubs=[],coaches=[],squads=[],memberships=[],clock=()=>new Date().toISOString()}={}){
    ...
    portal.register({id:'session-intake-flow',version:VERSION,kind:'application',purpose:'Explicit coordinator for schedule occurrence -> draft -> canonical accepted session -> schedule binding',owner:'session intake workflow',calls:{query:{'session-schedule':['occurrence','identityForOccurrence'],'session-lifecycle':['getDraft'],'session-truth':['parse','validate']},command:{'session-schedule':['linkSlots','bindSession','retireOccurrence'],'session-lifecycle':['createDraft','updateDraft','discardDraft','createFromDraft','markSuperseded']}},commands:{beginFromSlots:input=>call(I.intake,'beginFromSlots',input),beginFromOccurrence:input=>call(I.intake,'beginFromOccurrence',input),updateDraft:input=>call(I.intake,'updateDraft',input),discardDraft:input=>call(I.intake,'discardDraft',input),acceptDraft:input=>call(I.intake,'acceptDraft',input)}});
