...
(function(root,factory){
  ...
  function create({scheduleStorage,lifecycleStorage,attendanceStorage,captureStorage,deliveryStorage,calendarSources=[],evidenceSources=[],evidenceAliases=[],clubs=[],coaches=[],squads=[],memberships=[],profiles=[],overrides=[],standards=[],baseTimes=[],clock=()=>new Date().toISOString()}={}){
    ...
    portal.register({id:'results-pathway',version:E.ResultsPathway.VERSION,purpose:'PB and pathway presentation from verified evidence',owner:'performance pathway',calls:{query:{'evidence-retrieval':['resolveAthlete','results'],'standards-records':['normalizeCourse','normalizeStroke','normalizeEvent','isParaAthlete','paraClassification','matches','forEvent','baseTime','points','pointSteps','statusForResult','milestones']}},queries:{profile:input=>call(I.pathway,'profile',input),eventAnswer:input=>call(I.pathway,'eventAnswer',input),pbRows:input=>call(I.pathway,'pbRows',input)}});
    portal.register({id:'targets',version:E.Targets.VERSION,purpose:'Training targets from canonical set and evidence',owner:'training targets',calls:{query:{'evidence-retrieval':['resolveAthlete','latestTrainingTestEvidence','personalBestEvidence','conversion']}},queries:{forItem:input=>call(I.targets,'forItem',input),forPhase:input=>call(I.targets,'forPhase',input),t400:input=>call(I.targets,'t400',input),convertedPb:input=>call(I.targets,'convertedPb',input)}});
    portal.register({id:'adaptation',version:E.Adaptation.VERSION,purpose:'Athlete-specific prescription without mutating team truth',owner:'adaptation prescription',calls:{query:{'entity-registry':['resolveAthlete']}},queries:{forItem:input=>call(I.adaptation,'forItem',input),forAthletes:input=>call(I.adaptation,'forAthletes',input),profile:input=>call(I.adaptation,'profile',input),listOverrides:input=>call(I.adaptation,'listOverrides',input)},commands:{setOverride:input=>call(I.adaptation,'setOverride',input),clearOverride:input=>call(I.adaptation,'clearOverride',input)}});
    portal.register({id:'attendance',version:E.Attendance.VERSION,purpose:'Exact-session attendance truth',owner:'attendance records',calls:{query:{'entity-registry':['resolveAthlete','listAthletes']}},queries:{eligibleRoster:input=>call(I.attendance,'eligibleRoster',input),here:input=>call(I.attendance,'here',input),hereAthletes:input=>call(I.attendance,'hereAthletes',input),summary:input=>call(I.attendance,'summary',input),status:input=>call(I.attendance,'status',input)},commands:{mark:input=>call(I.attendance,'mark',input),clearMark:input=>call(I.attendance,'clearMark',input)}});
    portal.register({id:'capture-evidence',version:E.CaptureEvidence.VERSION,purpose:'Session/block/set/athlete evidence capture',owner:'capture evidence records',calls:{query:{'entity-registry':['resolveAthlete']}},queries:{atBoardPoint:input=>call(I.capture,'atBoardPoint',input),query:input=>call(I.capture,'query',input),get:input=>call(I.capture,'get',input)},commands:{create:input=>call(I.capture,'create',input),amend:input=>call(I.capture,'amend',input),retire:input=>call(I.capture,'retire',input)}});
    ...
    I.pathway=E.ResultsPathway.create({evidence:evidenceFor('results-pathway'),standardsEngine:proxy(portal.client('results-pathway'),'standards-records',['normalizeCourse','normalizeStroke','normalizeEvent','isParaAthlete','paraClassification','matches','forEvent','baseTime','points','pointSteps','statusForResult','milestones'])});
    ...
    const boardClient=portal.client('board-projection'),truthForBoard={nodeDistance:node=>boardClient.query('session-truth','nodeDistance',{node}),blockDistance:block=>boardClient.query('session-truth','blockDistance',{block}),totalDistance:session=>boardClient.query('session-truth','totalDistance',{session})};
    I.board=E.BoardProjection.create({truth:truthForBoard,attendance:proxy(boardClient,'attendance',['here','hereAthletes','summary']),adaptation:proxy(boardClient,'adaptation',['forItem']),targets:proxy(boardClient,'targets',['forItem','forPhase']),captures:proxy(boardClient,'capture-evidence',['atBoardPoint'])});
