...
(function(root){
  ...
  function setupPoolside(){
    state.poolRuntime=PoolsideRuntime.create({app:state.app});
    state.presenter=PoolsidePresenter.create({host:state.panelRoot});
    state.poolActions=PoolsideActions.create({runtime:state.poolRuntime,onChange:()=>render(),present:panel=>state.presenter.show(panel)});
    state.commandOwners=CommandOwners.create({runtime:state.poolRuntime,openers:state.poolActions.openers()});
    state.boardController=BoardController.create({root:state.root,commands:state.commandOwners.commands(),onError:error=>setNotice(error.message)}).bind();
  }
  async function boot(){
  ...
  function render(){if(!state.root||!state.nav||!state.shell)return;try{
    const route=state.nav.route();if(route.type!=='board'&&state.presenter?.panel)state.presenter.close();
    if(route.type==='calendar')return wrap(Renderer.renderCalendar(state.shell.calendar(route.month)),'Coach · Calendar');
    if(route.type==='day')return wrap(Renderer.renderDay(state.shell.day(route.date)),'Coach · Day');
    if(route.type==='board'){const occ=state.app.occurrenceForSession(route.sessionId);if(!occ||occ.id!==route.occurrenceId)throw new Error('Board route no longer matches its scheduled occurrence');const selected=state.app.selectedSession();if(selected?.id!==route.sessionId)throw new Error('Board route does not match selected session; reopen it from the Day view');const model=state.app.boardForSession(route.sessionId),html=BoardRenderer.renderBoard(model),delivery=state.app.deliveryForSession(route.sessionId);return wrap(Renderer.renderBoard({boardHtml:html,occurrence:occ,delivery}),'Coach · Board')}
    if(route.type==='detail'){const frame=state.nav.current();if(frame.detailType==='session-intake'){const day=state.shell.day(frame.date),item=day.items.find(x=>x.id===frame.detailId);if(!item)throw new Error('Session intake item is no longer on this date');return wrap(Renderer.renderIntake({item,availableDaySlots:frame.meta.availableDaySlots||[],selectedSlotIds:frame.meta.selectedSlotIds||[] }),'Coach · Session intake')}
      if(frame.detailType==='session-source'){const draft=state.app.portal.client('calendar-surface').query('session-lifecycle','getDraft',{args:[frame.detailId]});if(!draft)throw new Error('Session draft is no longer available');return wrap(Renderer.renderTextEntry({draftId:draft.id,title:draft.identity?.title,source:draft.source}),'Coach · Session entry')}
    }
  ...
  function internalBack(){if(state.presenter?.panel){state.presenter.close();return{handled:true,panel:true}}const result=state.nav.back();if(result.handled){persistNavigation();render()}return result}
  function onClick(event){const button=event.target.closest('[data-app-action]');if(!button)return;const action=button.dataset.appAction;try{
    ...
    if(action==='open-date'){state.shell.selectDate(button.dataset.date);persistNavigation();return render()}
    if(action==='open-item'){const result=state.shell.openItem(button.dataset.itemId);if(result.action==='board')state.app.selectSession(result.item.sessionId);persistNavigation();return result.action==='board'?render():openUnboundItem(result)}
    if(action==='intake-text'){
      const ids=selectedSlotIds();if(!ids.length)throw new Error('Choose at least one squad/session slot');const frame=state.nav.current(),day=state.shell.day(frame.date),first=day.items.find(x=>ids.includes(x.id)),title=`${first?.dayPart||''} · ${(ids.map(id=>day.items.find(x=>x.id===id)?.squadEntries?.[0]?.squadLabel).filter(Boolean)).join('+')}`.trim();const begun=state.app.beginFromSlots(ids,{inputMode:'text',title});state.nav.openDetail({detailType:'session-source',detailId:begun.draft.id,date:frame.date,occurrenceId:begun.occurrence.id,meta:{}});persistNavigation();return render()}
    if(action==='cancel-draft'){const frame=state.nav.current();if(frame.detailType==='session-source')state.app.discardDraft(frame.detailId,{retireOccurrence:true,note:'Coach cancelled session intake'});backToDay();persistNavigation();return render()}
  ...
  function onBrowserBack(){if(!state.nav)return;state.backGuard=false;if(state.presenter?.panel){state.presenter.close();rearmBrowserBack();return}if(state.nav.canBack()){state.nav.back();persistNavigation();render();rearmBrowserBack();return}history.back()}
  ...
  root.addEventListener('DOMContentLoaded',()=>boot().catch(fatal),{once:true});root.MSOSAssemblyBrowser={VERSION,state,boot,render,internalBack,onBrowserBack,setupPoolside};
