...
**2.24a Distinguish a rep's own internal breakdown from a genuine additive ladder of separate swims.** (Confirmed with Andy 17 Sept 2026, after his own real "400 #1 / 75 Drill / 25 Swim" session triggered exactly this question — see `msos-known-issues.md`.) Rule 2.24's "counts once" only applies when the smaller lines are a breakdown of the SAME already-counted work: their distances divide evenly into the parent's per-rep distance and repeat to reach it (e.g. `400 #1` swum as 4 × (75 Drill + 25 Swim) — 75+25=100, ×4=400 — is still 400m total, not 500m; matches the existing `6×50` = `25 drill/25 swim` example exactly, just with the "4 x" repeat left unwritten). It does NOT apply to a genuine descending/mixed-stroke ladder of separate, differently-sized swims that don't relate to each other or to a parent that way — e.g. `400 Fr / 300 Bk / 200 Br / 100 Fr` is four distinct swims totalling 1,000m, full stop, because nothing here is a repeating unit of anything above it. Test: does the smaller material divide evenly into a parent (or per-rep) distance and repeat to reach it? Yes → already-counted breakdown, 0 extra. No → real additive work, full metres each line. Do not guess this from the numbers alone without checking this test — and do not ask Andy to re-explain this distinction; it is now written down here.
...
**2.26a `#N` is a general rep-index prefix with (at least) three distinct real uses — don't conflate them.** (Found 17 Sept 2026 while fixing the 4×400 rotation-modification bug — see `msos-known-issues.md`.) `#1 Stroke` (2.26/2.40) is specifically the literal word "Stroke" after `#1`, meaning "fill in this athlete's own best/primary stroke here" — athlete-specific, resolved per swimmer. But `#N <anything else>` is a plain, general "this is what rep/round N does" annotation with no athlete-specific resolution involved — e.g. a following line `#1 IM` / `#2 Freestyle` / `#3 IM` / `#4 Choice` under a `4 x 400` means round 1 is IM, round 2 is Freestyle, etc. (real case, matches the existing `#1 Build, #2–6 @100m RP` example at line ~740 below). A THIRD use is a per-rep interval override, e.g. `#1 @ 1:00` / `#2 @ 1:15` / `#3 @ 1:30` under `3 x 50 #1 @ 200 Pace` — the label after `#N` is itself a send-off time (starts with `@`), not a stroke/content label, and must stay on its own cue-rendering path (tested in `tests/friday-session-board-regression.cjs`), not be captured as a content pattern. `app.js`'s `patternCue()` now handles all three: zone-cue lines (`#1 Regeneration`), content-pattern lines (`#1 IM`) feed `item.pattern` (which `engines/modification.js`'s `hasDistinctRepPattern()` reads to decide reps-vs-distance reduction for a rotation set), and `@`-led lines are excluded from pattern capture so they stay as plain cues. When touching this area, check which of the three a real line is before assuming it belongs to `item.pattern`.
