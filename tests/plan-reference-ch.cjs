...
assert.match(mon.session_focus,/Kick \/ Skills/);
assert.equal(mon.objective,undefined,'daily role must not replace the weekly direction');
...
assert.match(ctx.technicalFocus,/Race finishes/);
