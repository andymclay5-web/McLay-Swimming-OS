...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{
    ...
    M.state.settings.view='meet';M.store.save(M.state);M.ui.renderMeet();
  });
  await page.waitForSelector('[data-meet-workspace-cy]');await page.locator('[data-mwm-meet="south-real"]').click();await page.waitForTimeout(250);
  const s=await page.evaluate(()=>({id:MSOS4.state.settings.currentMeetId,deck:MSOS4.state.meetFieldDeck,sources:MSOS4.state.meetProgramBA.sources.length}));
  assert.equal(s.id,'south-real');assert.equal(s.deck,null);assert.equal(s.sources,0);console.log('MEET_EXPLICIT_SELECTION_AUTHORITY_PASS');
