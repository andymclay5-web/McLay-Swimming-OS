...
assert.equal(global.MSOS4.parserSemantics.cycle('8 x 25 on45'),45);assert.equal(global.MSOS4.parserSemantics.cycle('4 x 200 on 3.45'),225);assert.equal(global.MSOS4.parserSemantics.rest('Makeup: 50 Drill / 25 Swim · Rest · 10 sec'),10);
const wednesday=`WEDNESDAY PM — AEROBIC CAPACITY
...
Easy Choice`;
const wed=global.MSOS4.parser.parse(wednesday,{id:'wed-pm-20260826',course:'SCM',squads:['National']});assert.equal(S.total(wed),3500,'Wednesday known distance must be 3,500m before unspecified warm-down');const main=wed.blocks.find(b=>b.type==='main_set');assert.ok(main,'Wednesday main set block missing');assert.equal(S.blockDistance(main),1600,'2 Rounds must apply to all 400 + 200 + 200 work');const group=main.items.find(i=>i.kind==='group');assert.ok(group,'standalone 2 Rounds declaration must create a group');assert.equal(group.rounds,2);const roundSets=group.items.filter(i=>i.kind==='set');assert.deepEqual(roundSets.map(i=>[Number(i.reps),Number(i.distance)]),[[1,400],[1,200],[1,200]]);assert.deepEqual(roundSets.map(i=>Number(i.restSeconds)),[10,10,10]);assert.equal(wed.originalPlan.text,wednesday,'authored Wednesday text must remain immutable');assert.equal(wed.currentSource.text,wednesday,'parser preparation must not rewrite coach-visible source');assert.equal(S.blockDistance(wed.blocks.find(b=>b.type==='post_set')),1000);console.log('WEDNESDAY_STANDALONE_ROUNDS_PASS total=3500 main=1600 children=400+200+200');
console.log('PARSER_SEMANTICS_PASS');
