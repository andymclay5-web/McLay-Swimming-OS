...
function dataSources(){return[{id:'verified',priority:100,trust:'verified',data:{
 athletes:[
...
function evidence(){const sources=dataSources(),entities=Entities.create({sources});return{ev:Evidence.create({sources,entities}),entities}}
function system(src,{id=identity.id}={}){
 const {ev,entities}=evidence(),attendance=Attendance.create({storage:new Attendance.MemoryStorage(),evidence:entities,clock:(()=>{let n=0;return()=>`2026-08-18T05:30:${String(n++).padStart(2,'0')}+12:00`})()}),targets=Targets.create({evidence:ev}),adaptation=Adaptation.create({evidence:entities}),captures=Capture.create({storage:new Capture.MemoryStorage(),evidence:entities,clock:(()=>{let n=0;return()=>`2026-08-18T06:00:${String(n++).padStart(2,'0')}+12:00`})()}),board=Board.create({truth:Truth,attendance,adaptation,targets,captures}),session=Truth.parse(src,{...identity,id});return{entities,ev,attendance,targets,adaptation,captures,board,session};
}
