...
test('real standards are translated onto the same internally consistent WA points ladder',()=>{const current=Math.trunc(1000*Math.pow(51.71/63.8,3)),m=s.milestones(molly,{course:'SCM',distance:100,stroke:'Freestyle'},{result_seconds:63.8},{asOfDate:'2026-08-18',pointStepCount:2}),q=m.steps.find(x=>x.id==='qual-next');assert.equal(m.current.value,current);assert(q.points>current);assert(m.steps.some(x=>x.type==='points_step'&&x.points===550));assert(m.steps.some(x=>x.type==='record'&&x.id==='record'))});
