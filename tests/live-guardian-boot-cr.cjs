...
assert.doesNotMatch(nav,/stopImmediatePropagation/,'navigation must not seize clicks through capture-phase stopImmediatePropagation');
assert.doesNotMatch(nav,/\.bottom-nav \[data-nav\]/,'navigation engine must not duplicate the bottom-nav click owner');
...
assert.match(storage,/if\(useIndexed\)[\s\S]*?queueFull\(state\);\s*saveUi\(state\)/,'indexed operational state must bypass synchronous full JSON serialization');
