...
(async()=>{
 ...
 try{
  ...
  // Real named swimmers, current anchors and roll states.
  ...
  // Board remains a whiteboard: main work + aligned modifications + genuine target times.
  ...
  // TV is a read-only projection of the same canonical session.
  ...
  const thomas=live.find(x=>x.key==='thomas');await page.selectOption('#cnAthlete',thomas.id);await page.waitForFunction(()=>document.querySelector('#athletesView')?.innerText.includes('Thomas Cave'));let athleteText=await page.locator('#athletesView').innerText();assert.match(athleteText,/MY SWIMMING/);assert.match(athleteText,/PB|PATHWAY|QUALIFIER/i);
  ...
  // Jordan assistant contract: assigned-squad coaching, no owner/canonical authority.
  // 10 Sept 2026: assistant caps deliberately extended (engines/access-authority.js) so a real assistant
  // coach like Jordan can run/edit/finish sessions day to day -- edit/finish now assert true. The actual
  // security boundary this contract protects is squad scoping (names/denied below), which is unchanged.
  const jordan=await page.evaluate(()=>{const M=MSOS4;M.state.settings.assistantSquads=['National'];M.state.settings.assistantPermissions=[];M.access.setRole('assistant',{assistantId:'jordan'});M.state.settings.view='board';M.ui.renderCurrent();let denied=false;try{M.meet.assertRaceScope('accept-jordan-dev')}catch{denied=true}return{role:M.access.role(),names:M.access.visibleAthletes().map(a=>a.full_name),edit:M.access.can('session.edit'),finish:M.access.can('session.finish'),timing:M.access.can('timing.write'),attendance:M.access.can('attendance.write'),denied,nav:[...document.querySelectorAll('.bottom-nav button')].map(x=>x.textContent.trim())}});assert.equal(jordan.role,'assistant');assert.ok(jordan.names.includes('Matthew Kofoed'));assert.ok(!jordan.names.includes('Jordan Scope Development Swimmer'));assert.equal(jordan.edit,true);assert.equal(jordan.finish,true);assert.equal(jordan.timing,true);assert.equal(jordan.attendance,true);assert.equal(jordan.denied,true);assert.ok(!jordan.nav.some(x=>/Coach Hub/i.test(x)));
  await page.evaluate(()=>{MSOS4.access.setRole('owner');MSOS4.state.settings.assistantSquads=[];MSOS4.ui.renderCurrent()});

  // Meet Deck: programme -> plan -> live race -> split -> result/evidence.
  ...
  // Exact partial-finish truth must drive dose, then finish the full session for reporting.
  ...
  // Coach Hub and reports expose delivered energy-system/stroke dose, not just metres.
  ...
  // Persistence, Android Back and full offline reopen preserve the active finished session.
