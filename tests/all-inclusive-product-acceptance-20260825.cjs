...
(async()=>{
 ...
 try{
  ...
  const thomas=live.find(x=>x.key==='thomas');const swimmerSelector=page.locator('#cnAthlete');assert.equal(await swimmerSelector.count(),1,'Unified swimmer surface missing its athlete selector');await swimmerSelector.selectOption(thomas.id);await page.waitForFunction(()=>document.querySelector('#athletesView')?.innerText.includes('Thomas Cave'));let athleteText=await page.locator('#athletesView').innerText();assert.match(athleteText,/MY SWIMMING/);assert.match(athleteText,/PERFORMANCE ORDER/);assert.match(athleteText,/PB|pathway|next step/i);
