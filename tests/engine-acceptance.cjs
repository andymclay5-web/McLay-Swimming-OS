...
// A fair squad comparator needs at least three like-for-like swimmers. Attendance is not enough by itself.
...
// Modified IM is protected as a complete event unit; with a fair comparator it receives proportional work:rest and a group-connected rep count.
...
// Generic long work no longer means "cut reps and keep an interval the swimmer cannot make". Prefer same starts with shorter work when evidence is absent.
const timed75=set('timed75',4,75,{raw:'4 x 75 Pull @ 1:45',cycleSeconds:105});x=Modification.adaptItem(timed75,cm,state,session);assert.equal(x.reps,4);assert.equal(x.distance,50);assert.equal(x.cycleSeconds,105);assert.match(x.raw,/4 × 50/);
// Short quality can stay entirely with the group when the common interval still protects the stimulus.
...
const s75=set('s75',8,75,{raw:'8 x 75 with Fins 50 technique / 25 fast @ 1:45',cycleSeconds:105,equipment:['Fins']});x=ActiveModification.adaptItem(s75,md,state,session);assert.equal(x.reps,8);assert.equal(x.distance,50);assert.equal(x.cycleSeconds,105);x=ActiveModification.adaptItem(s75,ap,state,session);assert.equal(x.reps,8);assert.equal(x.distance,50);assert.equal(x.cycleSeconds,105);assert.equal(x.adaptivePending,true);assert.match(x.raw,/Upper-body choice/i);
