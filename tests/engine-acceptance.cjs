...
const kick=set('kick',5,50,{raw:'5 x 50 Kick Build @ 1:00',cues:['Kick Build'],cycleSeconds:60});x=Modification.adaptItem(kick,cm,state,session);assert.equal(x.reps,3);assert.equal(x.cycleSeconds,135);assert.equal(x.kickCycleRange?.min,130);assert.equal(x.kickCycleRange?.max,140);x=Modification.adaptItem(kick,cf,state,session);assert.equal(x.reps,3);assert.equal(x.cycleSeconds,100);x=Modification.adaptItem(kick,ap,state,session);assert.equal(x.reps,5);assert.equal(x.cycleSeconds,60);assert.match(x.raw,/Upper-body equivalent/i);
...
// The production runtime deliberately loads the AT Amber-alignment owner after the base Modification engine.
// Engine acceptance must exercise that same active chain rather than testing a stale pre-AT function in isolation.
...
const s75=set('s75',8,75,{raw:'8 x 75 with Fins 50 technique / 25 fast',equipment:['Fins']});x=ActiveModification.adaptItem(s75,md,state,session);assert.equal(x.reps,6);assert.equal((x.reps*x.distance/25)%2,0);x=ActiveModification.adaptItem(s75,ap,state,session);assert.equal(x.reps,6);assert.match(x.raw,/Upper-body/i);
