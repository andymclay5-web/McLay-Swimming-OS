...
const imState={...state,athletes:[cm,md,g1,g2,g3],resultsPbBoard:[...state.resultsPbBoard,{athlete_id:g1.id,distance:100,stroke:'IM',course:'SCM',result_seconds:68},{athlete_id:g2.id,distance:100,stroke:'IM',course:'SCM',result_seconds:70},{athlete_id:g3.id,distance:100,stroke:'IM',course:'SCM',result_seconds:72},{athlete_id:md.id,distance:100,stroke:'IM',course:'SCM',result_seconds:112},{athlete_id:cm.id,distance:100,stroke:'IM',course:'SCM',result_seconds:132},{athlete_id:g1.id,distance:100,stroke:'Backstroke',course:'SCM',result_seconds:62},{athlete_id:g2.id,distance:100,stroke:'Backstroke',course:'SCM',result_seconds:64},{athlete_id:g3.id,distance:100,stroke:'Backstroke',course:'SCM',result_seconds:66}]};
x=Modification.adaptItem(im,md,imState,session);assert.equal(x.distance,100);assert.equal(x.reps,3);assert.equal(x.cycleSeconds,175);assert.match(x.cyclePolicy||'',/athlete work|squad-window/i);assert.match(x.raw,/2:55/);assert.equal(x.imPerformancePlan.referenceSeconds,70);assert.equal(x.imPerformancePlan.athleteSeconds,112);assert.equal(x.imPerformancePlan.groupWindowSeconds,525);assert.equal(x.imPerformancePlan.totalSeconds,525);assert.ok(x.imPerformancePlan.recoveryFloorSeconds>=15);
x=Modification.adaptItem(im,cm,imState,session);assert.equal(x.reps,3);assert.equal(x.cycleSeconds,175);assert.match(x.raw,/2:55/);assert.equal(x.imPerformancePlan.athleteSeconds,132);assert.equal(x.imPerformancePlan.totalSeconds,525);assert.ok(x.cycleSeconds-x.imPerformancePlan.athleteWorkSeconds>=x.imPerformancePlan.recoveryFloorSeconds);
const phoneIm=set('phone-im',3,100,{stroke:'IM',raw:'3 x 100 IM @ 1:45 Descend 1-3',cues:['Descend 1-3'],cycleSeconds:105});
x=Modification.adaptItem(phoneIm,cm,imState,session);assert.equal(x.reps,2);assert.equal(x.distance,100);assert.equal(x.cycleSeconds,160);assert.match([x.raw,...x.cues].join(' '),/1 Build \/ 1 Fast/i);assert.equal(x.imPerformancePlan.groupWindowSeconds,315);assert.equal(x.imPerformancePlan.totalSeconds,320);
const strokeOverrideState={...imState,adaptationOverrides:[{sessionId:'fixture',itemId:'phone-im',athleteId:'cm',active:true,patch:{stroke:'Backstroke'}}]};
x=Modification.adaptItem(phoneIm,cm,strokeOverrideState,session);assert.equal(x.stroke,'Backstroke');assert.equal(x.reps,2);assert.equal(x.cycleSeconds,160);assert.equal(x.relativeStimulusEvidence.stroke,'Backstroke');assert.equal(x.relativeStimulusEvidence.referenceSeconds,64);assert.equal(x.relativeStimulusEvidence.athleteSeconds,70);
...
const devNoEvidence=set('dev-none',3,200,{zone:'Development',stroke:'Freestyle',raw:'3 x 200 Freestyle Development',restSeconds:10});t=Aerobic.forItem(session,devNoEvidence,{id:'none'},state);assert.equal(t.status,'fallback');assert.equal(t.hr,'<140');assert.equal(t.sr,'~30');
...
const board=fs.readFileSync(require.resolve('../engines/board.js'),'utf8');assert.match(board,/msos-mod-target/);assert.match(board,/function modified\(/);assert.match(board,/data-msos-ath/);assert.match(board,/function timingIntent\(/);assert.match(board,/m\.showTiming/);assert.doesNotMatch(board,/host\.className='view active msos-whiteboard-engine'/);
