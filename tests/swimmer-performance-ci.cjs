...
const standards=[
  {id:'past-nags-15',programme:'NAGS',season:'2026',active:true,standard_kind:'qualifying',course:'SCM',sex:'M',age_group:'15',age_min:15,age_max:15,distance:200,stroke:'IM',qualifying_seconds:143.52,meet_date:'2026-04-08'},
  {id:'future-nzsc-16-im2',programme:'NZSC',season:'2026',active:true,standard_kind:'qualifying',course:'SCM',sex:'M',age_group:'16',age_min:16,age_max:16,distance:200,stroke:'IM',qualifying_seconds:140.80,meet_date:'2026-09-27'},
  {id:'future-nzsc-15-im2',programme:'NZSC',season:'2026',active:true,standard_kind:'qualifying',course:'SCM',sex:'M',age_group:'15',age_min:15,age_max:15,distance:200,stroke:'IM',qualifying_seconds:143.31,meet_date:'2026-09-27'},
  {id:'future-nzsc-16-im4',programme:'NZSC',season:'2026',active:true,standard_kind:'qualifying',course:'SCM',sex:'M',age_group:'16',age_min:16,age_max:16,distance:400,stroke:'IM',qualifying_seconds:305.37,meet_date:'2026-09-27'},
...
global.MSOS4={
  ...
  performanceEngine:{
    scoreSystem:()=> 'WA',
    rankedEvents:()=>results.filter(r=>r.athlete_id===athlete.id).reduce((m,r)=>{const k=`${r.course}|${r.distance}|${r.stroke}`,old=m.get(k);if(!old||r.result_seconds<old.result_seconds)m.set(k,{course:r.course,distance:r.distance,stroke:r.stroke,seconds:r.result_seconds,points:r.wa_points,pointSystem:'WA'});return m;},new Map()).values?Array.from(results.filter(r=>r.athlete_id===athlete.id).reduce((m,r)=>{const k=`${r.course}|${r.distance}|${r.stroke}`,old=m.get(k);if(!old||r.result_seconds<old.seconds)m.set(k,{course:r.course,distance:r.distance,stroke:r.stroke,seconds:r.result_seconds,points:r.wa_points,pointSystem:'WA'});return m;},new Map()).values()).sort((a,b)=>b.points-a.points||a.seconds-b.seconds):[],
    invalidate:()=>{}
  },
...
const model=X.modelFor(athlete,'SCM');
...
assert.equal(im400.next.seconds,305.37,'400 IM must carry the correct current NZSC target');
