...
test('session evidence types are summarized and retain empty block/item address',()=>{const html=Renderer.renderSessionEvidence({count:3,byType:{photo:1,voice:1,note:1}},'s2');assert(/1 photo · 1 voice · 1 note/.test(html));assert(/data-session-id="s2"/.test(html));assert(/data-block-id=""/.test(html));assert(/data-item-id=""/.test(html));assert(!/data-block-id="[^\"]+"/.test(html));assert(!/data-item-id="[^\"]+"/.test(html))});
