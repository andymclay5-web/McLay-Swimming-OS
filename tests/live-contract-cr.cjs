...
const targetForItem=(session,item,ath,state)=>{const raw=clean([item?.raw,item?.text,...(item?.cues||[])].join(' '));if(/\bHR\s*Gauge\b/i.test(raw))return{status:'none',reason:'HR Gauge'};const zone=/\b(?:Regeneration|Development|Overload|Threshold|Clearance|Aerobic|VO2)\b/i.test(raw);if(zone&&!item?.targetSeconds&&!item?.raceIntent)return{status:'missing',message:'No Freestyle T400 target loaded'};return{status:'none'};};
...
global.MSOS4={
  ...
  guardian:{run:()=>({build:'old',tests:[
    ...
    {name:'Swimmer deck access is a header pill contract',ok:false,detail:'old API'},
    {name:'Poolside swimmer answer links real milestones to recent training area',ok:false,detail:'old API'},
    {name:'Poolside pathway answer is driven by loaded meet/benchmark milestones',ok:false,detail:'old API'},
  ...
  ],passed:2,total:9,ok:false})},
  store:{save:s=>s},storageEngine:{saveUi:()=>{}},
  targets:{t400:()=>null,forItem:targetForItem},
  ...
  swimmerTabsUI:{build:'v4-swimmer-deck-only-20260824cp',installDeckPills:()=>{}},
...
assert.match(bridge,/const suppressed=E\.Coordinator\.suppress\?\.\(item\)/,'bridge must consult Coordinator suppression before evidence guard');
const suppressAt=bridge.indexOf('const suppressed=E.Coordinator.suppress?.(item)'),zoneAt=bridge.indexOf('if(zoneIntent(item)',suppressAt);assert.ok(suppressAt>=0&&zoneAt>suppressAt,'suppression must run before no-evidence zone guard');
...
const hr=M.targets.forItem({identity:{course:'SCM'}},{kind:'set',reps:4,distance:100,stroke:'IM',zone:'Clearance',raw:'4 x 100 IM Clearance · HR Gauge',text:'4 x 100 IM Clearance · HR Gauge',cues:[]},{id:'none'},{trainingTestTypes:[],trainingTestResults:[]});
assert.equal(hr.status,'none');assert.match(hr.reason,/HR Gauge/i);
const run=M.guardianRuntime.fullRun();
assert.equal(run.build,'v4-guardian-live-contract-20260824cs');
for(const oldName of ['Swimmer hub is five simple tabs','Swimmer deck access is a header pill contract','Poolside swimmer answer links real milestones to recent training area','Poolside pathway answer is driven by loaded meet/benchmark milestones'])assert.equal(run.tests.some(x=>x.name===oldName),false,oldName);
...
assert.equal(run.tests.find(x=>x.name==='Current live target contract · HR Gauge suppresses pace even when Clearance is authored')?.ok,true);
...
console.log('LIVE_CONTRACT_CS_PASS',run.passed+'/'+run.total);
