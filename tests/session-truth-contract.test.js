...
test('Tuesday AM canonical distance is exactly 5400',()=>{
 ...
 for(const [type,metres] of Object.entries(expected))assert.equal(E.blockDistance(s.blocks.find(b=>b.type===type)),metres,type);
});

test('12 x 50 Total is summary metadata, never runnable distance',()=>{
 const s=E.parse(tuesday,id),pre=s.blocks.find(b=>b.type==='pre_set');
 assert(pre.items.some(x=>x.role==='summary'&&x.summaryMetres===600));
 assert.equal(E.blockDistance(pre),600);
});

test('pre-set remains one four-round group',()=>{
 const s=E.parse(tuesday,id),pre=s.blocks.find(b=>b.type==='pre_set'),groups=pre.items.filter(x=>x.kind==='group');
 assert.equal(groups.length,1);assert.equal(groups[0].rounds,4);assert.equal(E.nodeDistance(groups[0]),600);
});

test('post-set parent remains 16 x 50 and counts once',()=>{
 const s=E.parse(tuesday,id),post=s.blocks.find(b=>b.type==='post_set');
 assert.equal(post.items.filter(x=>x.kind==='set').length,1);
 const p=post.items.find(x=>x.kind==='set');assert.equal(p.reps,16);assert.equal(p.distance,50);assert.equal(E.nodeDistance(p),800);
 assert.equal(p.pattern.length,2);
 assert.equal(p.pattern[0].count,8);assert(/Bands/i.test(p.pattern[0].text));
 assert.equal(p.pattern[1].count,8);assert(/Swim/i.test(p.pattern[1].text));
});

test('rest-only line never creates phantom metres',()=>{
 const s=E.parse('Main Set\n6 x 100 Freestyle Development\n10s Rest',id),x=s.blocks[0].items[0];
 assert.equal(E.totalDistance(s),600);assert.equal(x.restSeconds,10);
});

test('15m Max is cue not distance',()=>{
 const s=E.parse('Pre Set\n4 x 25 #1\n15m Max',id),x=s.blocks[0].items[0];
 assert.equal(E.totalDistance(s),100);assert(x.cues.includes('15m Max'));
});

test('parent composition counts once',()=>{
 const s=E.parse('Warm Up\n4 x 300\n200 Free\n100 Reverse IM',id),x=s.blocks[0].items[0];
 assert.equal(E.totalDistance(s),1200);assert.equal(x.composition.length,2);
});

test('heading rounds multiply local work once',()=>{
 const s=E.parse('Main Set 3 rounds\n5 x 100 Free Threshold 10 sr\n400 Easy',id);
 assert.equal(E.totalDistance(s),2700);assert.equal(s.blocks[0].items[0].kind,'group');assert.equal(s.blocks[0].items[0].rounds,3);
});

test('blank line ends rounds scope',()=>{
 const s=E.parse('Main Set\n2 Rounds:\n4 x 100 Free Threshold 10 sr\n200 Easy\n\n100 Easy Reset',id);
 assert.equal(E.totalDistance(s),1300);assert.equal(s.blocks[0].items.length,2);
});
...
test('unknown coaching language is retained verbatim',()=>{
 ...
 assert(x.cues.includes(line));assert.equal(E.totalDistance(s),500);
});

test('17 Aug live session remains 4650',()=>{
 const src=`Warm up\n200 fr\n200 IM\n4x50 hbs\n10 sr\n\nPre set\n5x50 #1 build on 60\n5x100 IM desc 1-5 on 1.45\n\nMain set 3 rounds\n5x100 free threshold 10 sr\n400 easy\n\nPost set\n8x75\n25 Easy\n25 Build\n25 Fast\n\n4650m`;
 const s=E.parse(src,id);assert.equal(E.totalDistance(s),4650);assert.equal(s.metadata.totalMatches,true);
});
