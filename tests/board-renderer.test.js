...
test('4220 Board renders five compact blocks with grouped rounds and one-tap block navigation',()=>{
 const {board,session}=system(fixture),html=Render.renderBoard(board.project(session));
 assert(/4,220m/.test(html));assert.equal((html.match(/class="msos-board-block"/g)||[]).length,5);assert((html.match(/2 ROUNDS/g)||[]).length>=2);
 for(const name of ['Warm-up','Pre-set','Main set','Post-set','Warm-down'])assert(new RegExp(`>${name}<`).test(html),name);
 assert.equal((html.match(/href="#msos-board-block-/g)||[]).length,5);
});

test('common work renders once and genuine modification sits in side rail with athlete Edit',()=>{
 ...
 assert.equal((html.match(/400 Pull/g)||[]).length,1);assert.equal((html.match(/class="msos-mod-rail"/g)||[]).length,1);assert(/McKenzie/.test(html));assert(/data-board-action="edit-athlete"/.test(html));assert(/data-athlete-id="mk"/.test(html));
});

test('target is rendered directly under its exact set with compact source badge and full provenance retained',()=>{
 ...
 assert(/msos-targets/.test(html));assert(/>T400</.test(html));assert(/Latest valid Freestyle T400/.test(html));
...
test('sticky poolside actions keep Roll Times Capture Voice Photo Video Finish reachable',()=>{
 const {board,session}=system('Main Set\n4 x 25 Max @ 1:00'),html=Render.renderBoard(board.project(session));
 for(const action of ['roll','times','capture','voice','photo','video','finish'])assert(html.includes(`data-board-action="${action}"`),action);assert(/msos-board-sticky-shell/.test(html));
});
...
test('present attendance uses tiny swimmer names without repeating the word present',()=>{
...
test('phone CSS stacks modification rail keeps sticky shell and wraps target chips',()=>{
 const css=fs.readFileSync(require.resolve('../ui/board.css'),'utf8');assert(/position:\s*sticky/.test(css));assert(/@media\s*\(max-width:\s*720px\)/.test(css));assert(/\.msos-mod-rail\s*\{[^}]*grid-template-columns:/s.test(css));assert(/\.msos-targets\s*\{[^}]*display:\s*flex[^}]*flex-wrap:\s*wrap/s.test(css));
});
