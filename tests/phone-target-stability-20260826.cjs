...
(async()=>{
  ...
  try{
    ...
    assert.match(setup.boardStateBuild,/instant-times-a$/,'Instant Times Board state owner is not loaded');
    const prewarm=await page.evaluate(()=>MSOS4.boardStateEngine.prewarm());
    assert.equal(prewarm.complete,true,`Target prewarm did not complete: ${JSON.stringify(prewarm)}`);
    const prewarmed=await page.evaluate(id=>[...MSOS4.boardStateEngine.targetJobs.values()].filter(j=>j.itemId===id&&j.status==='done').length,setup.itemId);
    assert.equal(prewarmed,1,`Expected exactly one completed prewarmed job for target item, got ${prewarmed}`);
    ...
    await page.waitForFunction(()=>document.querySelectorAll('[data-msos-target-matrix] .msos-target-card').length===8,{timeout:500});
    const tapElapsed=Date.now()-tapStart;
    assert.ok(tapElapsed<200,`Prewarmed Times tap was not effectively instant: ${tapElapsed}ms`);
    ...
    assert.equal(noiseResult.done,1,`Prewarmed target job did not remain complete through hydratedAt churn: ${JSON.stringify(noiseResult)}`);
    ...
    console.log(`PHONE_TARGET_STABILITY_PASS tap=${tapElapsed}ms jobs=${noiseResult.jobs}`);
