...
(async()=>{
  ...
  try{
    ...
    const setup=await page.evaluate(()=>{
      ...
      return{sessionId:s.id,itemId:item?.id||''};
    });
    assert.ok(setup.itemId,'Target test item missing');
    const row=page.locator(`#boardView [data-item="${setup.itemId}"]`);assert.equal(await row.count(),1,'Target row missing');
    ...
    const beforeStroke=await row.evaluate(el=>el.getBoundingClientRect().top);
    const stroke=page.locator('[data-msos-target-matrix] [data-msos-fast-stroke]').first();assert.equal(await stroke.count(),1,'Stroke control missing from target matrix');await stroke.click();
    const bk=page.locator('.msos-stroke-menu button',{hasText:'Bk'});assert.equal(await bk.count(),1,'Backstroke choice missing');await bk.click();
    ...
    const afterRow=page.locator(`#boardView [data-item="${setup.itemId}"]`);const afterStroke=await afterRow.evaluate(el=>el.getBoundingClientRect().top);
    assert.ok(Math.abs(afterStroke-beforeStroke)<=4,`Stroke change moved Board row ${beforeStroke.toFixed(1)} -> ${afterStroke.toFixed(1)}`);
