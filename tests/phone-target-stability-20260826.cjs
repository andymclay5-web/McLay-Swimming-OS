...
(async()=>{
  ...
  try{
    ...
    console.log('PHONE_TARGET_BUILD',setup.boardStateBuild);
    ...
    const snap=label=>page.evaluate(({selector,label})=>{const r=document.querySelector(selector),p=r?.querySelector('.msos-target-panel'),m=document.querySelector('.msos-stroke-menu');const rr=r?.getBoundingClientRect(),pr=p?.getBoundingClientRect(),mr=m?.getBoundingClientRect();return{label,top:rr?.top??null,docY:rr?rr.top+scrollY:null,scrollY,scrollHeight:document.documentElement.scrollHeight,panelH:pr?.height??0,menuTop:mr?.top??null,menuBottom:mr?.bottom??null,menuPos:m?getComputedStyle(m).position:null};},{selector,label});
    ...
    const beforeStroke=await row.evaluate(el=>el.getBoundingClientRect().top),stages=[];stages.push(await snap('before-pill'));
    const stroke=page.locator('[data-msos-target-matrix] [data-msos-fast-stroke]').first();assert.equal(await stroke.count(),1,'Stroke control missing from target matrix');await stroke.click();stages.push(await snap('after-pill'));
    const bk=page.locator('.msos-stroke-menu button',{hasText:'Bk'});assert.equal(await bk.count(),1,'Backstroke choice missing');stages.push(await snap('before-bk'));await bk.click();stages.push(await snap('after-bk-click'));
    await page.waitForFunction(id=>{const s=MSOS4.currentSession(),o=(MSOS4.state.adaptationOverrides||[]).find(x=>x.sessionId===s.id&&x.itemId===id&&x.active!==false);return o?.patch?.stroke==='Backstroke';},setup.itemId,{timeout:3000});stages.push(await snap('after-override'));
    await page.waitForTimeout(120);stages.push(await snap('after-settle'));
    console.log('PHONE_SCROLL_STAGES',JSON.stringify(stages));
    const afterRow=page.locator(selector),afterStroke=await afterRow.evaluate(el=>el.getBoundingClientRect().top);
    assert.ok(Math.abs(afterStroke-beforeStroke)<=4,`Stroke change moved Board row ${beforeStroke.toFixed(1)} -> ${afterStroke.toFixed(1)} · ${JSON.stringify(stages)}`);
    assert.ok(await page.evaluate(()=>scrollY>0),'Stroke change returned Board to top');
