...
(async()=>{
  ...
  try{
    ...
    const stroke=page.locator('[data-msos-target-matrix] [data-msos-fast-stroke]').first();assert.equal(await stroke.count(),1,'Stroke control missing from target matrix');const sb=await stroke.boundingBox();assert.ok(sb&&sb.y>=0&&sb.y+sb.height<=844,`Stroke pill is not touch-visible: ${JSON.stringify(sb)}`);await page.touchscreen.tap(sb.x+sb.width/2,sb.y+sb.height/2);
    await page.waitForSelector('.msos-stroke-menu');assert.equal(await page.evaluate(()=>scrollY),beforeScroll,'Opening the stroke chooser scrolled the Board');
    const bk=page.locator('.msos-stroke-menu button',{hasText:'Bk'});assert.equal(await bk.count(),1,'Backstroke choice missing');const bb=await bk.boundingBox();assert.ok(bb&&bb.y>=0&&bb.y+bb.height<=844,`Backstroke choice is not touch-visible: ${JSON.stringify(bb)}`);await page.touchscreen.tap(bb.x+bb.width/2,bb.y+bb.height/2);
    ...
    assert.ok(Math.abs(afterStroke-beforeStroke)<=4,`Touch stroke change moved Board row ${beforeStroke.toFixed(1)} -> ${afterStroke.toFixed(1)}`);
    assert.equal(afterScroll,beforeScroll,`Touch stroke change changed scrollY ${beforeScroll} -> ${afterScroll}`);
