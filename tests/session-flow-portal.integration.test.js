...
test('accepted intake parses once through Session Truth, creates one Lifecycle session, and binds exact occurrence',()=>{
  const {core}=fixture(),slots=core.slotsForDate('2026-08-18'),begun=core.beginFromSlots(slots.map(x=>x.id),{inputMode:'text',title:'Tuesday AM'});core.updateDraft(begun.draft.id,{source:sessionSource});const accepted=core.acceptDraft(begun.draft.id);assert.strictEqual(accepted.record.id,accepted.session.id);assert.strictEqual(accepted.bound.sessionId,accepted.record.id);assert.strictEqual(core.sessions().length,1);assert.strictEqual(core.draft(begun.draft.id),null);
  const scheduleTruth=core.occurrenceForSession(accepted.record.id);assert.strictEqual(scheduleTruth.id,begun.occurrence.id);assert.strictEqual(scheduleTruth.squadEntries.find(x=>x.squadId==='squad-development').startOffsetMinutes,10);
  assert.deepStrictEqual(accepted.session.identity.squads,['National','Development']);assert.strictEqual(accepted.session.identity.start,'05:20');assert.strictEqual(accepted.session.identity.end,'07:20');assert.strictEqual(accepted.session.originalSource.text,sessionSource);assert.strictEqual(Object.prototype.hasOwnProperty.call(accepted.session.identity,'scheduleEntries'),false,'Schedule-owned squad offsets leaked into Session Truth');
});
