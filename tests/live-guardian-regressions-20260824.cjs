...
const version=fs.readFileSync(path.join(__dirname,'..','VERSION.txt'),'utf8').trim(),m=version.match(/McLay Swimming OS Version 4 · (\S+)/);assert.ok(m,'VERSION.txt declares release build');const expectedBuild=m[1];
global.MSOS4={BUILD:'stale-build',CORE:'old',release:{}};require('../engines/release-authority.js');assert.equal(global.MSOS4.BUILD,expectedBuild);assert.equal(global.MSOS4.RELEASE_ATTESTATION.build,global.MSOS4.BUILD);assert.equal(global.MSOS4.RELEASE_ATTESTATION.softwareReady,true);
