...
global.MSOS4={
  ...
  guardian:{run:()=>({build:'old',tests:[
    ...
    {name:'Engine · Reduced IM uses performance-relative send-off and stays connected to the group set window',ok:false,detail:'old 170 expectation'},
    ...
    {name:'Phone acceptance · McKenzie 50 kick keeps authored cycle and Desc 1-3',ok:false,detail:'Desc 1-3 lost'},
  ...
  swimmerExperienceCL:{disabled:true},
  performanceEngine:{pathwayUIck:{disabled:true}},
...
assert.equal(k.reps,8);
assert.equal(k.cycleSeconds,70);
assert.match([k.raw,...(k.cues||[])].join(' | '),/Desc 1-3/i);
assert.doesNotMatch((k.cues||[]).join(' | '),/Desc 1-8/i);
...
assert.equal(target.status,'missing','zone work without evidence must never get a fake pace target');

const run=M.guardianRuntime.fullRun();
assert.equal(run.build,'v4-guardian-live-contract-20260824cq');
assert.equal(run.tests.filter(x=>x.name.includes('Reduced IM')).length,0,'legacy Reduced IM contract should be retired');
assert.equal(run.tests.filter(x=>x.name==='Current reduced IM uses performance-relative send-off and stays connected to the group set window').length,1,'current Reduced IM contract must appear once');
...
console.log('LIVE_CONTRACT_CQ_PASS',run.passed+'/'+run.total);
