...
const board=fs.readFileSync(path.join(root,'engines/board.js'),'utf8');assert.ok(board.includes('if(hasRest&&('),'Board must suppress duplicate free-text rest cues when canonical restSeconds exists');assert.match(board,/\\bUnderwater\\b.*label\+=' Underwater'/s,'Board work label must preserve Underwater');assert.match(board,/canOpen=intent&&groupAth\.length>0/,'Board must not open empty target panels when no swimmers are present');
