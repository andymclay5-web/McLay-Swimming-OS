...
const html=text('swimmer-portal.html'),portal=text('swimmer-portal.js'),coach=text('engines/swimmer-invite-bn.js'),context=text('engines/swimmer-experience-cl.js'),instant=text('engines/swimmer-instant-open-cn.js'),sql=text('supabase/20260824_secure_swimmer_portal.sql'),index=text('index.html'),sw=text('sw.js');
...
assert.ok(portal.includes('Strongest performance first. Tap an event for pathway, PB race and splits.'),'portal must stay compact and event-led');
...
assert.ok(coach.includes("schema:'msos-swimmer-portal-v4'"),'secure payload schema did not advance with compact swimmer data');
