...
const q=label=>ladder.steps.find(s=>s.label===label&&s.kind==='qualifying');
...
assert.equal(q('NAGS').seconds,305.93,'next NAGS uses age-at-next-meet standard, not last age');
...
console.log('PERFORMANCE_PATHWAY_CK_PASS',ladder.next.label,'| NAGS',q('NAGS').targetSeason,'age',q('NAGS').ageAtTarget,'| NZSC final+medal loaded');
