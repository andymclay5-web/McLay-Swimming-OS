...
test('Roll command and Board read travel through the same portal and exact accepted session',()=>{const app=fixture(),accepted=buildAccepted(app);app.markAttendance(accepted.record.id,'molly','present');const board=app.boardForSession(accepted.record.id);assert.strictEqual(board.attendance.here,1);assert.strictEqual(board.attendance.athletes[0].athleteId,'molly');const trail=app.portal.auditTrail();assert(trail.some(x=>x.caller==='coach-board-surface'&&x.target==='attendance'&&x.operation==='mark'));assert(trail.some(x=>x.caller==='board-projection'&&x.target==='attendance'&&x.operation==='summary'))});
