...
(function(g){
  ...
  const R=M.guardianRuntime={build:'v4-guardian-runtime-20260820s'},fullRun=G.run.bind(G),baseSave=M.store.save.bind(M.store);
  ...
  const oldRender=M.ui?.renderGuardian?.bind(M.ui);if(oldRender)M.ui.renderGuardian=r=>{r=r||latestReal();if(r)return oldRender(r);const h=document.querySelector('#guardianView');if(h)h.innerHTML='<section class="page-card"><div class="eyebrow">GUARDIAN</div><h1>Full suite not run yet</h1><p>Guardian is deliberately excluded from the live poolside startup path. Run it explicitly when testing the build.</p><button id="rerunGuardian">Run Guardian</button></section>';h?.querySelector('#rerunGuardian')?.addEventListener('click',()=>G.runAndRender());};
