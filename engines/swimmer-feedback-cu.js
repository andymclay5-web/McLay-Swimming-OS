...
(function(g){
  ...
  const X=M.swimmerFeedbackCU={build:'v4-swimmer-feedback-20260824cu'};
  ...
  function install(){document.querySelector('#athletesView')?.addEventListener('click',e=>{if(e.target.closest?.('[data-cn-tab="training"]'))setTimeout(render,0);});if(M.state?.settings?.view==='athletes'&&M.state?.settings?.loopAthleteTab==='training')setTimeout(render,0);const s=document.createElement('style');s.textContent='.cu-feedback-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;padding:9px 0;border-top:1px solid #e4edf1;align-items:center}.cu-feedback-row>div{display:grid;gap:2px}.cu-feedback-row span{font-size:12px}.cu-feedback-row small{font-size:10px;color:#647a86}.cu-feedback-row button{font-size:11px}';document.head.append(s);}
