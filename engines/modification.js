    ...
    if(!manualShape&&p.ratio<.98){
      ...
      if(skill){out.adaptationReason='Same team exposure · independent skill quality';preserveAuthoredTiming(out,item,'Independent skill quality remains a common-start squad activity');}
        ...
        if(evidenceDistance){
          ...
          const ratio=evidence?.speedFactor||p.ratio,aerobicReturnToStart=p.returnToStart&&(key==='charlottemurphy'||key==='amberproudfoot'||Number(item.cycleSeconds)>0),desired=nearestPracticalDistance(baseDist*ratio,session,{returnToStart:aerobicReturnToStart,minDistance:Math.min(100,baseDist),maxDistance:baseDist});
          ...
          out.adaptationReason='Same team exposure · short work retained with squad · load recovered elsewhere';preserveAuthoredTiming(out,item,'Short work remains connected to the squad; global load is not enforced by cutting every small set');
        }else{
