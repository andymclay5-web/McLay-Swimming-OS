...
(function(root,factory){
  ...
  function trimRepMetadata(item){const x=item,max=Math.max(1,num(x?.reps)||1);if(Array.isArray(x.repInstructions))x.repInstructions=x.repInstructions.filter(r=>{const n=num(r?.rep);return n===null||n<=max});if(Array.isArray(x.repPattern))x.repPattern=x.repPattern.filter((r,i)=>{const n=num(r?.rep);return n===null?i<max:n<=max});if(x.raceIntent&&Array.isArray(x.raceIntent.reps))x.raceIntent.reps=x.raceIntent.reps.map(Number).filter(n=>Number.isFinite(n)&&n>=1&&n<=max);return x}
  ...
  class Adaptation{
    ...
    adaptItem(session,item,athleteRef,{profile:profileOverride=null,allowOverride=true}={}){
      ...
      if(profile.ratio<.98){
        if(preserveTeam){reasons.push('Same-team quality exposure preserved')}
        else if((x.phases||[]).length){x.phases=x.phases.map(p=>this.adaptPhase(session,x,p,ath,profile));x.reps=x.phases.reduce((n,p)=>n+(num(p.reps)||0),0);reasons.push('Phase structure preserved')}
      ...
      x=trimRepMetadata(x);const same=sameWork(item,x);return{status:'ok',sameAsGroup:same,prescription:x,reason:same?(reasons[0]||'Same as group'):reasons.join(' · ')||profile.label||'Athlete adaptation',profile,source:'derived'};
    }
