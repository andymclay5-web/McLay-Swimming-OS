...
(function(g){
  ...
  S.readyPromise.finally(()=>{released=true;clearTransient();requestAnimationFrame(()=>{if(!G.rendered)UI.renderCurrent();M.nav?.activateView?.(M.state?.settings?.view||'board');});pending=false;});
