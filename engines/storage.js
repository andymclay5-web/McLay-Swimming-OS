...
(function(g){
  ...
  const S=M.storageEngine={build:'v4-storage-20260820o'};
  ...
  function uiSnapshot(state){const s=state?.settings||{};return{view:s.view||'board',selectedSessionId:s.selectedSessionId||'',selectedAthleteId:s.selectedAthleteId||'',selectedSwimmerId:s.selectedSwimmerId||'',boardExpandedTargetId:s.boardExpandedTargetId||'',boardFocusMode:s.boardFocusMode!==false,boardBlockBySession:s.boardBlockBySession||{},t400Stroke:s.t400Stroke||'Freestyle',v4TimingMode:s.v4TimingMode||'t400',currentMeetId:s.currentMeetId||'',sessionScroll:s.sessionScroll||{},viewScroll:s.viewScroll||{},savedAt:Date.now()};}
  ...
  M.store.save=state=>{
    ...
    // Canonical/data saves are durable; transient Board/navigation state uses saveUi() and never serializes this full object.
