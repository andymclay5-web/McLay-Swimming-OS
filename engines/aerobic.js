...
(function(root,factory){const api=factory(root.MSOSEngines?.Evidence);if(typeof module==='object'&&module.exports)module.exports=api;else{root.MSOSEngines=root.MSOSEngines||{};root.MSOSEngines.Aerobic=api;}})(typeof globalThis!=='undefined'?globalThis:this,function(E){
  ...
  function forItem(session,item,ath,state,strokeOverride=''){
    ...
    const st=E?.stroke?.(strokeOverride||item?.stroke||'Freestyle')||'Freestyle';
    ...
    if(item?.repPattern?.length)return{status:'pattern',rows:item.repPattern.map(p=>({rep:p.rep,zone:p.zone,...target(sec,item.distance,p.zone,item.restSeconds??10)})),source:`${st} T400 ${clock(sec)}`,stroke:st};
    const r=target(sec,item.distance,item.zone,item.restSeconds??10);return r?{status:'ok',...r,source:`${st} T400 ${clock(sec)}`,stroke:st}:{status:'missing',message:'No valid T400 coefficient'}
  }
  return{VERSION,t400,hasAthleteEvidence,target,fallback,forItem,clock,RUSHTON};
});