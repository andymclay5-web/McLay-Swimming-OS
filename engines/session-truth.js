...
(function(root,factory){
  ...
  function speechToLines(source){
    const raw=String(source??'');if(/\n/.test(raw))return raw;if(!new RegExp(`\\b(?:${HEADING_WORD})\\b`,'i').test(raw))return raw;
  ...
  function heading(line){
    const t=text(line).replace(/[:]+$/,''),m=t.match(new RegExp(`^(${HEADING_WORD})\\s*(?:[-—:·]\\s*)?(\\d{1,2})\\s*rounds?\\b\\s*(.*)$`,'i'));
  ...
  const authoredRoundLine=line=>{const m=text(line).match(/^round\s+(\d{1,2})\s*:?\s*(.*)$/i);return m?{roundNumber:Number(m[1]),tail:text(m[2])}:null};
  ...
  function countInstruction(line){const t=text(line);if(!t||/^(?:\d{1,3})\s*(?:sr|s\s*r|s\s*rest|sec(?:onds?)?\s*rest|seconds?\s*rest|rest)$/i.test(t))return null;const m=t.match(/^(\d{1,2})\s+(.+)$/);if(!m)return null;const body=text(m[2]).replace(/^[-—:]\s*/,'');if(!body||/^(?:m|metres?|meters?|x\b|times?\b)/i.test(body))return null;return{count:Number(m[1]),text:body}}
  ...
  function explicitRepInstructions(raw,reps){
    ...
    const matches=[...src.matchAll(/#\s*(\d{1,3})/g)];
    const refs=matches.map(m=>Number(m[1])).filter(n=>n>=1&&n<=max);
    if(!refs.length)return[];
    const multiple=refs.length>1&&/\+/.test(src);
    const singleRepPrefix=/\brep\s*#\s*\d+/i.test(src);
    const single=refs.length===1&&(singleRepPrefix||refs[0]>1)&&/#\s*\d{1,3}\s*(?:@|at\b|pace\b|fast\b|max\b|easy\b|build\b|descend\b|race\b)/i.test(src);
  ...
  function instructionCompositionInfo(body,parentDistance){const t=text(body);if(!t.includes('/'))return{parts:[],span:0,repeats:0};const rawParts=t.split('/').map(text).filter(Boolean),parts=[];let span=0;for(const p of rawParts){const m=p.match(/^(\d{1,3}(?:\.5)?)\s*(?:m\b)?\s*(.*)$/i);if(!m)return{parts:[],span:0,repeats:0};const distance=Number(m[1]);span+=distance;parts.push({distance,text:text(m[2]),raw:p,stroke:strokeName(m[2]),equipment:equipment(m[2]),cycleSeconds:cycleSeconds(p),cycleOptions:cycleOptions(p),cues:[]})}const parent=Number(parentDistance)||0;if(!(span>0&&parent>=span&&Math.abs(parent/span-Math.round(parent/span))<1e-9))return{parts:[],span:0,repeats:0};return{parts,span,repeats:Math.round(parent/span)}}
  const instructionComposition=(body,parentDistance)=>instructionCompositionInfo(body,parentDistance).parts;
  function makePatternSegment(count,body,parentDistance=0,composition=null){const ci=composition?{parts:clone(composition),span:(composition||[]).reduce((n,x)=>n+Number(x.distance||0),0),repeats:1}:instructionCompositionInfo(body,parentDistance);if(composition&&ci.span>0&&parentDistance>=ci.span&&Math.abs(parentDistance/ci.span-Math.round(parentDistance/ci.span))<1e-9)ci.repeats=Math.round(parentDistance/ci.span);return{count:Number(count)||1,text:text(body),zone:zoneName(body),stroke:strokeName(body),equipment:equipment(body),raceIntent:raceIntent(body),drill:/\b(?:drill|scull|technique)\b/i.test(body),composition:ci.parts,compositionRepeats:ci.repeats||0,cycleSeconds:cycleSeconds(body),cycleOptions:cycleOptions(body),cues:[],repInstructions:[]}}
  ...
  function inlineCountPattern(raw,parentDistance,reps){const r=explicitRepeat(raw);if(!r||!r.tail||!r.tail.includes('/'))return[];const segments=r.tail.split(/[·;]/).map(text).filter(Boolean);for(const segment of segments){if(!segment.includes('/'))continue;const p=countPattern(segment,parentDistance);if(!p)continue;const count=p.reduce((n,x)=>n+Number(x.count||0),0);if(count>0&&count<=Number(reps)&&Number(reps)%count===0)return p}return[]}
  function makeSet(sessionId,blockTypeName,order,raw,reps,distance){const r=text(raw),opts=cycleOptions(r),n=Math.max(1,Number(reps)||1),set={id:stable('set',sessionId,blockTypeName,order,r),kind:'set',order,reps:n,distance:Math.max(0,Number(distance)||0),stroke:strokeName(r),zone:zoneName(r),restSeconds:restSeconds(r),cycleSeconds:opts[0]??null,cycleOptions:opts,equipment:equipment(r),raw:r,composition:parentheticalComposition(r,distance),compositionRepeats:1,pattern:[],patternRounds:null,phases:[],repPattern:[],cues:[],raceIntent:raceIntent(r),repInstructions:dedupeRepInstructions([...oddEvenInstructions(r,n),...explicitRepInstructions(r,n)]),targetSeconds:null};const inline=inlineCountPattern(r,distance,n);if(inline.length){set.pattern=inline;expandPattern(set)}return set}
  ...
  function looksLikeOpeningWork(rawLines){for(const raw of rawLines||[]){const t=text(raw);if(!t)continue;if(explicitRepeat(t)||diveStartSet('probe','warm_up',1,t))return true;const s=singleDistance(t);if(s&&s.distance>=25&&/(?:free|freestyle|choice|IM\b|back|breast|fly|pull|paddles|kick|swim|scull|HBS|regeneration|development|overload|threshold|clearance|easy|build)/i.test(s.tail||''))return true}return false}
  ...
  function parseBlock(sessionId,type,rawLines,blockRounds=null){
    ...
    for(const rawLine of rawLines){
      ...
      const authored=authoredRoundLine(line);if(authored){groups.length=0;const g={id:stable('group',sessionId,type,++order,`round-${authored.roundNumber}`),kind:'group',order,rounds:1,label:`Round ${authored.roundNumber}`,scope:'authored_round',roundNumber:authored.roundNumber,items:[]};root.push(g);groups.push(g);currentSet=null;lastRepeatGroup=null;if(authored.tail){const dive=diveStartSet(sessionId,type,++order,authored.tail);if(dive)push(dive);else{order--;const rep=explicitRepeat(authored.tail);if(rep)push(makeSet(sessionId,type,++order,authored.tail,rep.reps,rep.distance));else{const one=singleDistance(authored.tail);if(one&&one.distance>=10)push(makeSet(sessionId,type,++order,authored.tail,1,one.distance));else g.items.push({id:stable('cue',sessionId,type,++order,authored.tail),kind:'cue',order,text:authored.tail,raw:authored.tail})}}}continue}
      const rl=roundLine(line);if(rl){if(currentSet&&Number(currentSet.reps)>1&&rl.rounds>1&&Number(currentSet.reps)%rl.rounds===0&&rl.rounds<Number(currentSet.reps)){currentSet.patternRounds=rl.rounds;continue}const g={id:stable('group',sessionId,type,++order,line),kind:'group',order,rounds:rl.rounds,label:rl.tail||'',scope:'local_rounds',items:[]};list().push(g);groups.push(g);currentSet=null;continue}
      const repeat=repeatMarker(line);if(repeat&&repeat>1){const target=list(),start=groups.length?0:segmentStart,items=target.splice(start);if(items.length){const g={id:stable('group',sessionId,type,++order,`repeat-${repeat}-${items.map(x=>x.id).join('|')}`),kind:'group',order,rounds:repeat,label:'Repeat',scope:'repeat',items};target.push(g);lastRepeatGroup=g;currentSet=null}continue}
      const restOnly=line.match(/^(\d{1,3})\s*(?:sr|s\s*r|s\s*rest|sec(?:onds?)?\s*rest|seconds?\s*rest|rest)$/i);if(restOnly){const n=Number(restOnly[1]);if(currentSet)currentSet.restSeconds=n;else if(lastRepeatGroup)applyRest(lastRepeatGroup.items,n);continue}
      const allRest=line.match(/^all\s+(?:with\s+)?(\d{1,3})\s*(?:sr|s\s*r|s\s*rest|sec(?:onds?)?\s*rest|seconds?\s*rest|rest)(?:\s+period)?$/i);if(allRest){applyRest(list().slice(groups.length?0:segmentStart),Number(allRest[1]));continue}
      ...
      if(currentSet&&/\bodd\b/i.test(line)&&/\beven\b/i.test(line)){appendCue(currentSet,line);continue}
      if(currentSet&&line.includes('/')){const ci=instructionCompositionInfo(line,currentSet.distance);if(ci.parts.length){if(currentSet.patternRounds){currentSet.pattern.push(makePatternSegment(1,line,currentSet.distance,ci.parts));expandPattern(currentSet)}else if(!(currentSet.composition||[]).length){currentSet.composition=ci.parts;currentSet.compositionRepeats=ci.repeats}else appendCue(currentSet,line);continue}const counts=countPattern(line,currentSet.distance);if(counts&&counts.reduce((n,x)=>n+x.count,0)<=Math.max(1,Number(currentSet.reps)||1)){currentSet.pattern.push(...counts);expandPattern(currentSet);continue}}
      ...
      const counted=countInstruction(line);if(currentSet&&counted&&counted.count<=Math.max(1,Number(currentSet.reps)||1)){currentSet.pattern.push(makePatternSegment(counted.count,counted.text,currentSet.distance));expandPattern(currentSet);continue}
      const one=singleDistance(line);if(one&&Number(one.distance)>=10){if(one.hasUnit&&Number(one.distance)<25){if(currentSet)appendCue(currentSet,line);else list().push({id:stable('cue',sessionId,type,++order,line),kind:'cue',order,text:line,raw:line});continue}push(makeSet(sessionId,type,++order,line,1,one.distance));lastRepeatGroup=null;continue}
      if(currentSet){appendCue(currentSet,line);continue}list().push({id:stable('cue',sessionId,type,++order,line),kind:'cue',order,text:line,raw:line});
    }
    let out=foldChildren(root);const explicitRounds=out.filter(n=>n.kind==='group'&&n.scope==='authored_round');if(blockRounds&&blockRounds>1&&!explicitRounds.length)out=[{id:stable('group',sessionId,type,'block-rounds',blockRounds),kind:'group',order:1,rounds:blockRounds,label:'',scope:'block',items:out}];return out;
  }

  function createSession(identity,source){const id=identity?.id||stable('session',identity?.date||'',identity?.dayPart||'',identity?.title||'',source);return{schema:'msos.session.v4',engineVersion:VERSION,id,identity:{date:identity?.date||'',dayPart:identity?.dayPart||'',title:identity?.title||'',squads:[...(identity?.squads||[])],venue:identity?.venue||'',course:identity?.course||'',start:identity?.start||'',end:identity?.end||''},originalSource:{text:String(source??''),hash:hash(source)},blocks:[],metadata:{writtenTotal:null,parsedTotal:0,totalMatches:true,warnings:[]}}}
  function parse(source,identity={}){const normalized=normaliseSource(source),session=createSession(identity,source),chunks=[];let currentType=null,currentLines=[],blockRounds=null,writtenTotal=null,currentTitle='';const preamble=[];const flush=()=>{if(!currentType)return;chunks.push({type:currentType,lines:currentLines,authoredTitle:currentTitle,blockRounds});currentLines=[];blockRounds=null;currentTitle=''};for(const raw of lines(normalized)){const t=text(raw),total=t.match(/^TOTAL\s*[:=]?\s*([\d,]+)\s*m?$/i)||t.match(/^([\d,]{3,6})\s*m$/i);if(total){writtenTotal=Number(total[1].replace(/,/g,''));continue}const h=heading(t);if(h){flush();currentType=h.type;blockRounds=h.rounds||null;currentTitle=h.authoredTitle||BLOCK_TITLES[h.type]||'';if(h.tail)currentLines.push(h.tail);continue}if(!currentType){preamble.push(raw);continue}currentLines.push(raw)}flush();const blocks=[];const opening=looksLikeOpeningWork(preamble);if(opening){const items=parseBlock(session.id,'warm_up',preamble,null);if(items.reduce((n,x)=>n+nodeDistance(x),0)>0)blocks.push({id:stable('block',session.id,'warm_up','implicit'),type:'warm_up',title:'Warm-up',authoredTitle:'',implicit:true,sourceOrder:0,order:1,items})}const notes=opening?[]:preamble.filter(x=>text(x));for(const c of chunks){blocks.push({id:stable('block',session.id,c.type,blocks.length,c.authoredTitle),type:c.type,title:/post\s*[- ]?main/i.test(c.authoredTitle)?'Post-main set':BLOCK_TITLES[c.type]||c.authoredTitle||'Block',authoredTitle:c.authoredTitle||'',sourceOrder:blocks.length+1,order:blocks.length+1,authoredRounds:c.blockRounds||null,items:parseBlock(session.id,c.type,c.lines,c.blockRounds)})}blocks.forEach((b,i)=>{b.order=i+1;b.sourceOrder=i+1});session.blocks=blocks;session.metadata.normalizedSource=normalized;session.metadata.sessionNotes=notes;session.metadata.writtenTotal=writtenTotal;session.metadata.parsedTotal=totalDistance(session);session.metadata.totalMatches=writtenTotal==null||Math.abs(writtenTotal-session.metadata.parsedTotal)<=1;if(writtenTotal!=null&&!session.metadata.totalMatches)session.metadata.warnings.push(`Written total ${writtenTotal}m does not match parsed total ${session.metadata.parsedTotal}m`);return session}
  ...
  return{VERSION,parse,validate,totalDistance,blockDistance,nodeDistance,internals:{blockType,speechToLines,spokenWork,normaliseSource,normaliseNaturalLine,heading,roundLine,authoredRoundLine,repeatMarker,explicitRepeat,singleDistance,summaryRepeat,countInstruction,foldChildren,zoneName,strokeName,restSeconds,cycleSeconds,cycleOptions,raceIntent,explicitRepInstructions,oddEvenInstructions,parentheticalComposition,instructionComposition,instructionCompositionInfo,hyphenAllocation,inlineCountPattern,looksLikeOpeningWork}};
});
