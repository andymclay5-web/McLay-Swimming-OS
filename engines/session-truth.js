...
(function(root,factory){
  ...
  function spokenWork(fragment){
    ...
    if(m){
      const reps=wordNumber(m[1]),dw=m[2].toLowerCase().replace(/\s+/g,'');
  ...
  function normaliseSource(source){
    return lines(speechToLines(source)).flatMap(line=>normaliseNaturalLine(line).split('\n')).filter(x=>!/^and$/i.test(text(x))).join('\n');
  }
  ...
  function repeatMarker(line){
    let m=text(line).match(/^(?:repeat\s+(?:that\s+)?)?x\s*(\d{1,2})\s*$/i);if(m)return Number(m[1]);
    m=text(line).match(new RegExp(`^repeat\\s+(?:that\\s+)?(${NUMBER_WORD}|\\d+)\\s+times?$`,'i'));
    return m?wordNumber(m[1]):null;
  }
  const explicitRepeat=line=>{const m=text(line).match(/^(\d{1,3})\s*[x×]\s*(\d{1,4}(?:\.5)?)\b\s*(.*)$/i);return m?{reps:Number(m[1]),distance:Number(m[2]),tail:text(m[3])}:null};
  ...
  function countInstruction(line){
    ...
    if(!body||/^(?:m|metres?|meters?)\b/i.test(body))return null;
  ...
  function explicitRepInstructions(raw,reps){
    ...
    const multiple=refs.length>1&&/\+/.test(src),single=refs.length===1&&/#\s*\d{1,3}\s*(?:@|at\b|pace\b|fast\b|max\b|easy\b|build\b|descend\b|race\b)/i.test(src);
  ...
  function instructionComposition(body,parentDistance){
    const t=text(body);if(!t.includes('/'))return[];
    const parts=t.split('/').map(text).filter(Boolean),out=[];let sum=0;
    for(const p of parts){
      const m=p.match(/^(\d{1,3}(?:\.5)?)\s*(?:m\b)?\s*(.*)$/i);if(!m)return[];
      const distance=Number(m[1]);sum+=distance;
      out.push({distance,text:text(m[2]),raw:p,stroke:strokeName(m[2]),equipment:equipment(m[2]),cycleSeconds:cycleSeconds(p),cycleOptions:cycleOptions(p),cues:[]});
    }
    return Math.abs(sum-Number(parentDistance))<.001?out:[];
  }

  const makePatternSegment=(count,body,parentDistance=0,composition=[])=>({
    count:Number(count)||1,text:text(body),zone:zoneName(body),stroke:strokeName(body),equipment:equipment(body),
    raceIntent:raceIntent(body),drill:/\b(?:drill|scull|technique)\b/i.test(body),
    composition:composition.length?clone(composition):(parentDistance?instructionComposition(body,parentDistance):[]),
    cycleSeconds:cycleSeconds(body),cycleOptions:cycleOptions(body),cues:[],repInstructions:[]
  });
  ...
  function parentheticalComposition(raw,distance){
    const m=String(raw??'').match(/\(([^)]+)\)/);return m?instructionComposition(m[1],distance):[];
  }
  ...
  function makeSet(sessionId,blockTypeName,order,raw,reps,distance){
    ...
    return{
      ...
      equipment:equipment(r),raw:r,composition:parentheticalComposition(r,distance),pattern:[],patternRounds:null,phases:[],repPattern:[],cues:[],
      raceIntent:raceIntent(r),repInstructions:dedupeRepInstructions([...oddEvenInstructions(r,n),...explicitRepInstructions(r,n)]),targetSeconds:null
    };
  }
  ...
  function countPattern(line,parentDistance=0){
    ...
    const out=[];
    for(const p of parts){const m=p.match(/^(\d{1,2})\s+(.+)$/);if(!m)return null;out.push(makePatternSegment(Number(m[1]),m[2],parentDistance))}
    return out;
  }

  function childDescriptor(c){
    return{count:Number(c.reps)||1,reps:Number(c.reps)||1,distance:Number(c.distance)||0,text:text(c.raw).replace(/^\d{1,3}\s*[x×]\s*\d{1,4}(?:\.5)?\s*/i,''),raw:c.raw,stroke:c.stroke||'',zone:c.zone||'',equipment:clone(c.equipment||[]),restSeconds:c.restSeconds??null,cycleSeconds:c.cycleSeconds??null,cycleOptions:clone(c.cycleOptions||[]),raceIntent:clone(c.raceIntent),composition:clone(c.composition||[]),pattern:clone(c.pattern||[]),repInstructions:clone(c.repInstructions||[]),cues:clone(c.cues||[])};
  }
  ...
  function adoptComposition(parent,parts){
    parent.composition=parts.map(c=>({distance:setDistance(c),text:text(c.raw),raw:c.raw,stroke:c.stroke||'',equipment:clone(c.equipment||[]),cycleSeconds:c.cycleSeconds??null,cycleOptions:clone(c.cycleOptions||[]),cues:clone(c.cues||[])}));
    if(!parent.cycleSeconds){const cycles=[...new Set(parts.map(c=>c.cycleSeconds).filter(Number.isFinite))];if(cycles.length===1){parent.cycleSeconds=cycles[0];parent.cycleOptions=[cycles[0]]}}
  }
      ...
      if(Number(parent.reps)>=2){
        ...
        if(kids.length>=2&&count>0&&Number(parent.reps)%count===0){
          if(count===Number(parent.reps))parent.phases=kids.map(childDescriptor);
            ...
            parent.pattern=kids.map(c=>{const seg=makePatternSegment(Number(c.reps)||1,text(c.raw).replace(/^\d{1,3}\s*[x×]\s*\d{1,4}(?:\.5)?\s*/i,''),parent.distance);seg.cues=clone(c.cues||[]);seg.raceIntent=clone(c.raceIntent);seg.repInstructions=clone(c.repInstructions||[]);return seg});
        ...
        while(j<a.length){
          const c=a[j];if(c?.kind!=='set'||Number(c.reps)!==1||Number(c.distance)<=0||Number(c.distance)>=Number(parent.distance))break;
          parts.push(c);sum+=Number(c.distance);j++;if(sum>=Number(parent.distance))break;
        }
  ...
  function appendCue(set,line){
    const t=text(line);set.cues.push(t);
    const refs=explicitRepInstructions(t,set.reps);if(refs.length)set.repInstructions=dedupeRepInstructions([...(set.repInstructions||[]),...refs]);
  }
  ...
  function parseBlock(sessionId,type,rawLines,blockRounds=null){
    ...
    for(const rawLine of rawLines){
      ...
      if(rl){
        if(currentSet&&Number(currentSet.reps)>1&&rl.rounds>1&&Number(currentSet.reps)%rl.rounds===0&&rl.rounds<Number(currentSet.reps)){
          currentSet.patternRounds=rl.rounds;continue;
        }
        const g={id:stable('group',sessionId,type,++order,line),kind:'group',order,rounds:rl.rounds,label:rl.tail||'',items:[]};
      ...
      if(repeat&&repeat>1){
        ...
        if(items.length){const g={id:stable('group',sessionId,type,++order,`repeat-${repeat}-${items.map(x=>x.id).join('|')}`),kind:'group',order,rounds:repeat,label:'Repeat',items};target.push(g);lastRepeatGroup=g;currentSet=null}
        continue;
      }
      ...
      // Context-first interpretation: slash lines after a parent can be within-rep composition,
      // not new runnable metres. This must happen before generic numeric set parsing.
      if(currentSet&&line.includes('/')){
        const comp=instructionComposition(line,currentSet.distance);
        if(comp.length){
          if(currentSet.patternRounds){currentSet.pattern.push(makePatternSegment(1,line,currentSet.distance,comp));expandPattern(currentSet)}
          else if(!(currentSet.composition||[]).length)currentSet.composition=comp;
          else appendCue(currentSet,line);
          continue;
        }
        const counts=countPattern(line,currentSet.distance);
        if(counts&&counts.reduce((n,x)=>n+x.count,0)<=Math.max(1,Number(currentSet.reps)||1)){
          currentSet.pattern.push(...counts);expandPattern(currentSet);continue;
        }
      }

      const counted=countInstruction(line);
      if(currentSet&&counted&&counted.count<=Math.max(1,Number(currentSet.reps)||1)){
        currentSet.pattern.push(makePatternSegment(counted.count,counted.text,currentSet.distance));expandPattern(currentSet);continue;
      }
      ...
      if(one&&Number(one.distance)>=10){
        // Short markers such as 15m Max are instructions, not standalone work, unless
        // explicitly authored as repetitions (e.g. 4 x Dive Start to 15m).
        if(one.hasUnit&&Number(one.distance)<25){if(currentSet)appendCue(currentSet,line);else list().push({id:stable('cue',sessionId,type,++order,line),kind:'cue',order,text:line,raw:line});continue}
    ...
    if(blockRounds&&blockRounds>1){out=[{id:stable('group',sessionId,type,'block-rounds',blockRounds),kind:'group',order:1,rounds:blockRounds,label:'',scope:'block',items:out}]}
    return out;
  }
  ...
  function parse(source,identity={}){
    ...
    for(const raw of lines(normalized)){
      ...
      if(!currentType){if(t)notes.push(t);continue}
      currentLines.push(raw);
    }
    flush();

    session.blocks=chunks.map((c,i)=>({
      id:stable('block',session.id,c.type,i,c.authoredTitle),type:c.type,
      title:/post\s*[- ]?main/i.test(c.authoredTitle)?'Post-main set':BLOCK_TITLES[c.type]||c.authoredTitle||'Block',
      authoredTitle:c.authoredTitle||'',sourceOrder:i+1,order:i+1,items:parseBlock(session.id,c.type,c.lines,c.blockRounds)
    }));
