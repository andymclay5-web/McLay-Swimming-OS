...
(function(root,factory){
  ...
  const BLOCK_TITLES={warm_up:'Warm-up',pre_set:'Pre-set',main_set:'Main set',post_set:'Post-set',sharpness:'Sharpness',kick:'Kick',skill:'Skill',speed:'Speed',aerobic_reset:'Aerobic reset',test:'Test',warm_down:'Warm-down',other:'Other'};
  ...
  function spokenWork(fragment){
    let x=text(fragment).replace(/^(?:and\s+)?then\s+/i,'').replace(/^and\s+/i,'').replace(/\bwith\s+(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty)\s+seconds?\s+rest\b/ig,(_,w)=>`${wordNumber(w)}s rest`).replace(/\b(on|at)\s+a\s+minute\b/ig,'on 60');
    ...
    m=x.match(new RegExp(`^(${NUMBER_WORD}|\\d{1,2})\\s+(.+)$`,'i'));if(m&&!/^\d+(?:\.\d+)?\s*(?:m|metres?|meters?)\b/i.test(x)){const n=wordNumber(m[1]);if(Number.isFinite(n))return`${n} ${text(m[2])}`}
    return x;
  }
  ...
  function speechToLines(source){
    ...
    for(let sentence of sentences){sentence=sentence.replace(/^after\s+that\s+/i,'');const hm=sentence.match(new RegExp(`^(${HEADING_WORD})\\b\\s*(?:will\\s+be|is|:|,)?\\s*(.*)$`,'i'));if(!hm){out.push(sentence);continue}out.push(hm[1]);let body=text(hm[2]);if(!body)continue;body=body.replace(new RegExp(`,?\\s+and\\s+repeat\\s+that\\s+(${NUMBER_WORD}|\\d+)\\s+times?\\b`,'i'),(_,n)=>`, Repeat x${wordNumber(n)}`);const parts=body.split(/\s*,\s*(?:and\s+)?(?:then\s+)?|\s+(?:and\s+)?then\s+/i).map(text).filter(Boolean);for(const part of parts){const p=text(part).replace(/^and$/i,'');if(!p)continue;const rm=p.match(new RegExp(`^(?:and\\s+)?repeat\\s+(?:that\\s+)?(?:x\\s*)?(${NUMBER_WORD}|\\d+)(?:\\s+times?)?$`,'i'));out.push(rm?`Repeat x${wordNumber(rm[1])}`:spokenWork(p))}}
    return out.join('\n');
  }

  function normaliseNaturalLine(line){
    let x=String(line??'').trim();if(!x)return'';x=x.replace(/[×✕]/g,'x').replace(/[–—]/g,'—').replace(/\bon\s+a\s+minute\b/ig,'on 60');x=x.replace(/\b(\d{1,2})\s*x\s*10p\s+i[’']?m\b/gi,'$1 x 100 IM');x=x.replace(/^([1-9]\d?)(25|50|75|100|200|400)s\b/i,(_,r,d)=>`${r} x ${d}`);x=x.replace(new RegExp(`^(${NUMBER_WORD})\\s+times?\\s+(12\\.5|15|25|35|50|65|75|100|150|200|300|400|800|1500)\\b\\s*(.*)$`,'i'),(_,w,d,tail)=>`${wordNumber(w)} x ${d}${text(tail)?' '+text(tail):''}`);x=x.replace(/^(\d{1,3})\s+times?\s+(12\.5|15|25|35|50|65|75|100|150|200|300|400|800|1500)\b\s*(.*)$/i,(_,r,d,tail)=>`${r} x ${d}${text(tail)?' '+text(tail):''}`);x=x.replace(new RegExp(`^(${NUMBER_WORD})\\s+(25|50|75|100|200|400)s?\\b`,'i'),(_,w,d)=>`${wordNumber(w)} x ${d}`);x=x.replace(/^(\d{1,2})\s+(25|50|75|100|200|400)s\b/i,'$1 x $2');x=x.replace(new RegExp(`^(${NUMBER_WORD})\\s+(.+)$`,'i'),(_,w,tail)=>`${wordNumber(w)} ${tail}`);const wd=x.match(/^(warm\s*[- ]?down|cool\s*[- ]?down)\s+(?:of\s+)?(\d{2,4})\s*(?:m|metres?|meters?)?\s*$/i);if(wd)return`${wd[1]}\n${wd[2]}`;return x;
  }
  ...
  function heading(line){const t=text(line).replace(/[:]+$/,''),m=t.match(new RegExp(`^(${HEADING_WORD})\\s*(?:[-—:·]\\s*)?(\\d{1,2})\\s*rounds?\\b\\s*(.*)$`,'i'));if(m)return{type:blockType(m[1]),rounds:Number(m[2]),tail:text(m[3]),authoredTitle:text(m[1])};const exact=t.match(new RegExp(`^(${HEADING_WORD})$`,'i'));return exact?{type:blockType(exact[1]),rounds:null,tail:'',authoredTitle:text(exact[1])}:null}
  ...
  function parseBlock(sessionId,type,rawLines,blockRounds=null){
    ...
    for(const rawLine of rawLines){
      ...
      const rl=roundLine(line);if(rl){if(currentSet&&Number(currentSet.reps)>1&&rl.rounds>1&&Number(currentSet.reps)%rl.rounds===0&&rl.rounds<Number(currentSet.reps)){currentSet.patternRounds=rl.rounds;continue}const g={id:stable('group',sessionId,type,++order,line),kind:'group',order,rounds:rl.rounds,label:rl.tail||'',items:[]};list().push(g);groups.push(g);currentSet=null;continue}
      ...
      // Explicit runnable syntax takes precedence over child-count grammar.
