...
(function(g){
  ...
  const R=M.reportingEngine={build:'v4-reporting-20260820u'};
  ...
  function run(spec={},state=M.state){const role=M.access?.role?.()||'owner',scope=role==='swimmer'?'swimmer':(spec.scope||'squad'),days=spec.days===undefined||spec.days===null?7:Number(spec.days),course=spec.course||state?.settings?.pathwayCourse||'SCM',metrics=Array.isArray(spec.metrics)&&spec.metrics.length?spec.metrics:allMetrics().map(x=>x.id),athletes=filterAthletes({...spec,scope},state),rows=athletes.map(a=>athleteRow(a,state,days,course)),aggregate=aggregateStroke(rows),timedCount=rows.reduce((n,r)=>n+r.timedSets.length,0),t400Count=rows.reduce((n,r)=>n+Object.keys(r.t400||{}).length,0),captureTotal=rows.reduce((n,r)=>n+r.captureCount,0),refs=references();return{spec:{scope,days,course,squad:spec.squad||'',athleteId:role==='swimmer'?(state?.settings?.activeUserAthleteId||''):(spec.athleteId||''),metrics},generatedAt:new Date().toISOString(),rows,aggregate,references:refs,summary:{athletes:rows.length,rawMetres:aggregate.totalRaw,weightedFocus:aggregate.totalWeighted,t400Anchors:t400Count,recentTimedSets:timedCount,captures:captureTotal,rankedAthletes:rows.filter(r=>r.bestEventPoints!=null).length,pathwayMatched:rows.filter(r=>r.pathway?.event).length},methodology:{strokeWeights:M.strokeBalance.WEIGHTS,provisional:true,note:'Raw metres and weighted focus are both retained; incidental/easy Freestyle is deliberately discounted.'}};}
