...
(function(g){
  ...
  function run(spec={},state=M.state){const role=M.access?.role?.()||'owner',scope=role==='swimmer'?'swimmer':(spec.scope||'squad'),days=Number(spec.days||7),metrics=Array.isArray(spec.metrics)&&spec.metrics.length?spec.metrics:METRICS.map(x=>x.id),athletes=filterAthletes({...spec,scope},state),rows=athletes.map(a=>athleteRow(a,state,days)),aggregate=aggregateStroke(rows),timedCount=rows.reduce((n,r)=>n+r.timedSets.length,0),t400Count=rows.reduce((n,r)=>n+Object.keys(r.t400||{}).length,0),captureTotal=rows.reduce((n,r)=>n+r.captureCount,0);return{spec:{scope,days,squad:spec.squad||'',athleteId:role==='swimmer'?(state?.settings?.activeUserAthleteId||''):(spec.athleteId||''),metrics},generatedAt:new Date().toISOString(),rows,aggregate,summary:{athletes:rows.length,rawMetres:aggregate.totalRaw,weightedFocus:aggregate.totalWeighted,t400Anchors:t400Count,recentTimedSets:timedCount,captures:captureTotal},methodology:{strokeWeights:M.strokeBalance.WEIGHTS,provisional:true,note:'Raw metres and weighted focus are both retained; incidental/easy Freestyle is deliberately discounted.'}};}
  function squads(state=M.state){return[...new Set(visibleAthletes(state).map(a=>a.squad).filter(Boolean))].sort();}
  R.METRICS=METRICS;R.FIELDS=FIELDS;R.catalog=()=>({metrics:METRICS,fields:FIELDS,scopes:['squad','swimmer'],windows:[7,14,28,0],strokes:STROKES});R.run=run;R.squads=squads;R.athleteRow=athleteRow;R.aggregateStroke=aggregateStroke;R.visibleAthletes=visibleAthletes;
})(globalThis);
