...
(function(g){
  ...
  const R=M.reportingEngine={build:'v4-reporting-20260820ab'};
  ...
  const BASE_METRICS=[
    ...
    {id:'development',label:'Event coverage / development',source:'Development + Evidence + Performance'},
    ...
    {id:'reference_data',label:'Reference versions / data health',source:'Data Registry + Evidence'}
  ];
  const BASE_FIELDS=[
    ...
    {id:'pb_health',label:'PB / WA evidence health',source:'Evidence + WA Points'},
    {id:'development_opportunities',label:'Development event opportunities',source:'Development'},
    {id:'xlr8_monitor',label:'Junior XLR8 coverage monitor',source:'Development + Evidence'},
  ...
  function athleteRow(ath,state,days,course='SCM'){const perf=M.performanceEngine.profile(ath,state,course),bal=M.strokeBalance.summary(ath,state,{days}),best=perf.bestEvent,stroke=perf.bestStroke,form=perf.bestFormStroke,recentTimed=timedRows(ath,state,days),path=pathwayRow(ath,course),captures=captureRows(ath,state,days),development=M.developmentEngine?.profile?.(ath,state,course)||null,pbHealth=perf.pbs.length?(Number.isFinite(best?.points)?'ready':'pb_without_wa'):'no_pb',extras={};for(const [id,fn] of providers){try{extras[id]=fn({athlete:ath,state,days,course,performance:perf,strokeBalance:bal,pathway:path,captures,development});}catch{extras[id]=null;}}return{athlete:ath,athleteId:ath.id,name:ath.full_name,squad:ath.squad||'',course,bestEvent:best?`${best.distance} ${best.stroke}`:'No ranked PB',bestEventPoints:Number.isFinite(best?.points)?Math.floor(best.points):null,bestEventPointVersion:best?.pointVersion||'',bestStroke:stroke?.stroke||'',bestFormStroke:form?.stroke||'',medleyPrimary:!!perf.medleyPrimary,contextStroke:perf.contextStroke?.stroke||'',contextReason:perf.contextStroke?.source||'',strokeBalance:bal,t400:perf.t400,timedSets:recentTimed,pbCount:perf.pbs.length,pbHealth,pointStatus:perf.pointStatus,pathway:path,development,opportunityCount:development?.opportunities?.length||0,xlr8:development?.xlr8||null,sessionCount:bal.sessions,modifiedSessions:modifiedSessions(ath,state,days),captureCount:captures.length,captures,extras};}
