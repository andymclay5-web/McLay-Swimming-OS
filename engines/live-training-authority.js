...
(function(g){
  ...
  if(!M||!U||!L)return;
  const BUILD='v4-live-training-authority-20260901a';
  ...
  L.apply=msg=>{
    ...
    try{
      ...
      if(msg.sessionId){
        M.state.attendance=(M.state.attendance||[]).filter(x=>x.session_id!==msg.sessionId).concat(clone(msg.attendance||[]));
        M.state.adaptationOverrides=(M.state.adaptationOverrides||[]).filter(x=>x.sessionId!==msg.sessionId).concat(clone(msg.adaptationOverrides||[]));
      }
      ...
      if(view==='tv')M.ui?.renderTV?.();else if(view==='swimmer')M.ui?.renderSwimmer?.();
      return true;
    }finally{L.suppress=false;}
  };
  const style=document?.createElement?.('style');if(style){style.id='msos-training-surface-isolation';style.textContent='body:not([data-msos-view="meet"]) #meetModeBtn,body:not([data-msos-view="meet"]) .bottom-nav [data-nav="meet"]{display:none!important}';document.head?.appendChild?.(style);}
  M.liveTrainingAuthority={build:BUILD,operationalViews:[...operationalViews],derivedViews:[...derivedViews]};
})(globalThis);
