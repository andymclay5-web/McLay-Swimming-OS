...
(function(g){
  ...
  const B=M.strokeBalance={build:'v4-stroke-balance-20260820q'};
  ...
  function splitStroke(item,total){const out=[];const comp=item?.composition||[];if(comp.length){for(const c of comp){const st=strokeInText(c?.stroke||c?.text||'');if(STROKES.includes(st))out.push({stroke:st,metres:Number(c.distance)||0});else if(st==='IM')for(const s of STROKES)out.push({stroke:s,metres:(Number(c.distance)||0)/4});}const sum=out.reduce((n,x)=>n+x.metres,0);if(sum>0){const scale=total/sum;return out.map(x=>({...x,metres:x.metres*scale}));}}
    const st=strokeInText(item?.stroke||item?.raw||item?.text||'');if(st==='IM')return STROKES.map(s=>({stroke:s,metres:total/4}));if(STROKES.includes(st))return[{stroke:st,metres:total}];return[];
  }
