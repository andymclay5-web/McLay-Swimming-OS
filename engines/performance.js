...
(function(g){
  ...
  const P=M.performanceEngine={build:'v4-performance-20260820p'};
  ...
  function rankedEvents(ath,state=M.state,course=''){
    return rows(ath,state,course).filter(r=>Number.isFinite(r.points)&&r.points>0).sort((a,b)=>b.points-a.points||a.seconds-b.seconds);
  }
  ...
  function selectStrokeForContext(ath,item,state=M.state,session=null,{formOnly=false}={}){
    ...
    const ranked=bestStroke(ath,state,course,formOnly);if(!ranked)return{stroke:'',source:'No ranked PB evidence',confidence:'none'};
    const event=bestEvent(ath,state,course),isIM=event?.stroke==='IM';
    if(isIM&&M.strokeBalance?.recommendStroke){const r=M.strokeBalance.recommendStroke(ath,state,session,{formOnly});if(r?.stroke)return r;}
  ...
  function profile(ath,state=M.state,course=''){
    ...
    const medley=ev?.stroke==='IM';const context=medley?selectStrokeForContext(ath,null,state,{identity:{course}},{}):null;
