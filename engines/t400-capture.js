'use strict';
(function(g){const M=g.MSOS4,E=g.MSOSEngines;if(!M?.timing?.coreSaveT400||!E?.Aerobic||!E?.Evidence)return;const C=M.t400Capture={build:'v4-t400-capture-20260826-coherent'};const core=M.timing.coreSaveT400.bind(M.timing),clock=s=>M.util?.clock?.(Number(s))||String(s),now=()=>new Date().toISOString();
  ...
  M.timing.saveT400=function(athleteId,value,session=M.currentSession?.(),state=M.state,date,stroke='Freestyle',meta={}){
    const ath=athlete(athleteId,state),st=E.Evidence.stroke(stroke||'Freestyle'),prior=ath?E.Aerobic.t400(ath,state,st):null,priorSec=E.Evidence.seconds(prior),row=core(athleteId,value,session,state,date,st,meta),current=E.Evidence.seconds(row),cmp=comparison(current,priorSec),liveState=state===M.state;
