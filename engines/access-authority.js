...
(function(g){
  ...
  function setRole(next,{athleteId='',assistantId=''}={}){
    ...
    // Persist only after the entire identity binding is internally consistent.
  ...
  function visibleAthletes(){
    const seen=new Set(),out=[];for(const a of M.state.athletes||[]){if(a?.active===false||!athleteAllowed(a))continue;const id=String(a?.id||'');if(!id||seen.has(id))continue;seen.add(id);out.push(a);}return out;
  }
