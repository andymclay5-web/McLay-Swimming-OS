...
(function(g){
  ...
  function compactAthleteNotes(card){card.querySelectorAll('[data-loop-open-cap]').forEach(b=>{const c=(M.state?.captures||[]).find(x=>x.id===b.dataset.loopOpenCap);if(text(c?.capture_type).toLowerCase()!=='note')return;const strong=b.querySelector('b'),small=b.querySelector('small');if(strong)strong.textContent=text(c?.title||c?.capture_title)||'Note';small?.remove();});}
  ...
  function enhanceAthlete(){
    ...
    const panel=h.querySelector('[data-msos-ath-panel="performance"]');if(panel){panel.querySelectorAll('.perf-rank strong').forEach(el=>{const m=text(el.textContent).match(/^(\d+)\s+WA\b/i);if(m)el.textContent=`${m[1]} WA`;});const cards=[...panel.querySelectorAll(':scope > .page-card')],coverage=cards.find(c=>/Event coverage\s*\/\s*development/i.test(text(c.querySelector('h2')?.textContent)));collapseCard(coverage,'Development opportunities');}
  }
