...
(function(g){
  ...
  const CAM={
    'amber proudfoot':{
      '50|Freestyle':[31.07,33.24,30.79,32.97],'400|Freestyle':[308.94,329.99,300.13,315.65],'100|Backstroke':[80.35,86.01,78.36,82.89],'100|Breaststroke':[null,null,91.58,105.80],'100|Butterfly':[76.26,93.04,73.60,77.22],'200|IM':[169.94,186.37,161.29,173.10]
    },
    'mckenzie drage':{
      '100|Freestyle':[74.74,82.29,70.43,73.97],'400|Freestyle':[331.17,377.26,312.61,336.91],'100|Backstroke':[83.52,91.83,null,null],'100|Breaststroke':[76.73,81.00,84.50,88.15],'50|Butterfly':[36.86,38.06,35.40,37.74],'200|IM':[185.93,206.82,181.27,190.43]
    },
    'charlotte murphy':{
      '50|Freestyle':[34.34,37.88,33.01,35.95],'400|Freestyle':[319.69,375.35,321.36,361.73],'100|Backstroke':[82.00,95.39,82.24,95.02],'100|Breaststroke':[99.46,114.88,94.15,110.45],'50|Butterfly':[36.80,38.89,37.51,39.74],'200|IM':[182.96,197.48,183.60,208.28]
    },
    'ruby stace':{
      '50|Freestyle':[28.03,29.41,27.60,28.22],'100|Freestyle':[60.12,66.45,null,null],'400|Freestyle':[279.79,301.60,276.17,295.95],'100|Backstroke':[66.98,84.34,68.08,73.35],'100|Breaststroke':[78.45,83.08,78.52,83.82],'100|Butterfly':[66.18,71.27,65.43,67.53],'200|IM':[148.84,161.35,147.47,155.37]
    }
  };
  ...
  function derivedRows(pbSeconds,ref){
    return bandPercents(pbSeconds,ref.mqs).map((pct,i)=>({
      id:`para-mqs-band-${pct}`,_seconds:ref.mqs*(1+pct/100),_kind:'development',_label:`MQS +${pct}% development`,standard_kind:'performance',programme:`MQS +${pct}% development`,progression_order:200+i,para_class:ref.class,course:'LCM',source_status:'DERIVED_COACHING_MARKER',notes:'Derived from official Singapore 2025 MQS. Coaching progression marker only; not an official qualifying standard.'
    }));
  }
  function officialRows(pbSeconds,ref){const out=[];
    if(pbSeconds>ref.met+.001)out.push({id:'para-met',_seconds:ref.met,_kind:'qualifying',_label:'International MET · Singapore 2025',standard_kind:'qualifying',programme:'International MET · Singapore 2025',progression_order:160,para_class:ref.class,course:'LCM',source_status:'OFFICIAL_WPS'});
    out.push({id:'para-mqs',_seconds:ref.mqs,_kind:'qualifying',_label:'International MQS · Singapore 2025',standard_kind:'qualifying',programme:'International MQS · Singapore 2025',progression_order:170,para_class:ref.class,course:'LCM',source_status:'OFFICIAL_WPS'});return out;}
  function camRows(ath,pb){
    const a=CAM[norm(ath?.full_name)],row=a?.[eventKey(pb.distance,pb.stroke)];if(!row)return[];
    const labels=['Manchester podium benchmark','Manchester final benchmark','Paris podium benchmark','Paris final benchmark'];return row.map((sec,i)=>Number(sec)>0?{id:`cam-${i}`,_seconds:Number(sec),_kind:i%2===0?'medal':'finalist',_label:labels[i],standard_kind:'performance',programme:labels[i],progression_order:300+i,source_status:'CAMERON_LESLIE_TRACKING'}:null).filter(Boolean);
  }
  ...
  P.event=(ath,pb)=>{
    ...
    const qualifying=dedupe([...unachievedDomestic,...officialQ]);
    const deeper=dedupe([...(base.deeper||[]),...derived,...historic]);
    const allAhead=[...qualifying,...deeper].filter(r=>pbSec>Number(r._seconds)+.001).sort((a,b)=>((pbSec-a._seconds)/a._seconds)-((pbSec-b._seconds)/b._seconds));
    const next=allAhead[0]||null;
  ...
  P.profile=(ath,course='')=>{const profile=baseProfile(ath,course);if(!P.isPara?.(ath))return profile;const events=(profile.events||[]).map(e=>P.event(ath,e.pb));const closest=events.filter(e=>e.nextNational).sort((a,b)=>a.nextNational.gap.percentage-b.nextNational.gap.percentage)[0]||null;return{...profile,events,closest,paraInternational:true};};
  ...
  function decorate(){
    ...
    const h=head.querySelector('h2');if(h)h.textContent='International performance pathway';
  }
  const priorRender=UI.renderAthletes?.bind(UI);if(priorRender)UI.renderAthletes=()=>{priorRender();queueMicrotask(decorate);requestAnimationFrame(decorate);};
