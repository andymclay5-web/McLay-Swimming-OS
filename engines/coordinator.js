...
(function(root,factory){const api=factory(root.MSOSEngines||{});if(typeof module==='object'&&module.exports)module.exports=api;else{root.MSOSEngines=root.MSOSEngines||{};root.MSOSEngines.Coordinator=api;}})(typeof globalThis!=='undefined'?globalThis:this,function(E){
  const VERSION='2.2.0',text=v=>String(v??'').replace(/\s+/g,' ').trim();
  const cache=new Map();
  ...
  function peekTarget(session,item,ath,state){const ov=overrideStroke(item,ath,state,session),k=cacheKey(session,item,ath,state,ov);return cache.get(k)||null}
  ...
  return{VERSION,targetForItem,peekTarget,prescription,overrideStroke,clearCache};
});
