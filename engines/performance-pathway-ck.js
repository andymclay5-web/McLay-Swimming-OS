...
(function(g){
  ...
  function familyTrack(f){if(['nzsc','div2','regional_sc','nzss','world_sc'].includes(f))return'SCM';if(['nags','nzopen','world_lc','olympics'].includes(f))return'LCM';return'';}
  function trackFor(r){const f=family(programme(r)),fixed=familyTrack(f);if(fixed)return fixed;const c=courseOf(r);if(c==='SCM'||c==='LCM'||c==='BOTH')return c;return c||'BOTH';}
  ...
  function chooseProgrammeRows(rows,ath,event,viewCourse,now=today()){
    ...
    preferred=nativeRows(preferred,route);
