...
(function(g){
  ...
  const X=P.pathwayCK={build:'v4-performance-pathway-20260824ck'};
  ...
  function chooseProgrammeRows(rows,ath,event,viewCourse,now=today()){
    ...
    const dedupe=new Map();for(const r of preferred){const k=`${kind(r)}|${secondsOf(r).toFixed(2)}|${courseOf(r)}`;if(!Number.isFinite(secondsOf(r))||secondsOf(r)<=0||dedupe.has(k))continue;dedupe.set(k,{raw:r,label:programme(r),family:f,kind:kind(r),seconds:secondsOf(r),course:courseOf(r)||wanted,officialCourse:trackFor(r)==='LCM'?'LCM':trackFor(r)==='SCM'?'SCM':courseOf(r),targetDate:target.date,targetSeason:targetYr,sourceSeason,planningProxy:target.planningProxy,sourceDate:target.sourceDate,ageAtTarget:age,sourceStatus:text(r?.source_status||r?.source_version),sourceUrl:text(r?.source_url)});}
    return[...dedupe.values()];
  }
  function staticDeep(event,viewCourse){const rows=[...(event?.deeper||[])],out=[];for(const r of rows){const sec=Number(r?._seconds??M.pathway?.seconds?.(r));if(!Number.isFinite(sec)||sec<=0)continue;const k=kind(r);if(k==='qualifying')continue;const c=courseOf(r)||viewCourse;if(c&&c!=='BOTH'&&viewCourse&&c!==viewCourse)continue;out.push({raw:r,label:text(r?._label||M.pathway?.standardLabel?.(r)||programme(r)||'Benchmark'),family:family(r?._label||programme(r)),kind:k,seconds:sec,course:c||viewCourse,officialCourse:c||viewCourse,targetDate:'',targetSeason:null,sourceSeason:Number(r?.season)||null,planningProxy:false,sourceStatus:text(r?.source_status||r?.source_version),sourceUrl:text(r?.source_url)});}return out;}
