...
(function(g){
  ...
  function tests(){const out=[],ath={id:'amber-ap',full_name:'Amber Proudfoot'},session={id:'ap-session',identity:{course:'SCM'}};
    out.push(test('Amber non-target upper-body options keep the 2/3 load and authored interval',()=>{const x=A.adaptItem(item('ap-paddles'),ath,stateFor('ap-paddles','Paddles'),session);assert(x.reps===3,`reps ${x.reps}`);assert(Number(x.cycleSeconds)===60,`cycle ${x.cycleSeconds}`);return `5×50 @1:00 → ${x.reps}×50 @${M.util.clock(x.cycleSeconds)}`;}));
    ...
    return out;}
  G.run=()=>{const base=baseRun()||{},all=[...(base.tests||[]),...tests()],passed=all.filter(x=>x.ok===true).length;return{...base,build:BUILD,tests:all,passed,total:all.length,ok:all.length>0&&passed===all.length,contract:'20260823ca'};};
