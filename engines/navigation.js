...
(function(g){
  ...
  const N=M.nav,UI=M.ui,V=M.navigationEngine={build:'v4-navigation-20260820u'};
  ...
  N.dismissLayer=()=>{const layer=history.state?.layer;if(layer){closeTransient();saveUi();history.back();return true;}closeTransient();saveUi();UI.renderCurrent?.();renderExtra(M.state?.settings?.view||'board');return false;};
