...
(function(g){
  ...
  const N=M.nav,UI=M.ui,V=M.navigationEngine={build:'v4-navigation-20260820j'};
  ...
  V.go=(view,{push=true,restore=true}={})=>{
    ...
    active(view);M.store?.save?.(M.state);
    ...
    UI.renderCurrent?.();
    // Renderers may update DOM, but navigation owns which top-level view is visible.
    active(view);
    if(restore)try{N.restoreScroll?.(view)}catch{}else requestAnimationFrame(()=>scrollTo(0,0));
  };
  N.show=V.go;N.activateView=active;

  // Board quick links must cross the navigation bridge, never set view state directly.
  document.addEventListener('click',e=>{
    ...
    if(ath){e.preventDefault();e.stopImmediatePropagation();M.state.settings.selectedAthleteId=ath.dataset.msosAth;M.state.settings.selectedSwimmerId=ath.dataset.msosAth;V.go('swimmer',{restore:false});}
  },true);

  // Board/TV renderers are not allowed to claim top-level active state.
