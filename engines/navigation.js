...
(function(g){
  ...
  const N=M.nav,UI=M.ui,V=M.navigationEngine={build:'v4-navigation-20260825-first-tap'};
  ...
  const rememberScroll=()=>{try{M.state.settings=M.state.settings||{};M.state.settings.viewScroll=M.state.settings.viewScroll||{};M.state.settings.viewScroll[scrollKey()]=Math.max(0,Math.round(window.scrollY||0));}catch{}};
  ...
  const state=(view,extra={})=>({msos:true,msosView:view||M.state?.settings?.view||'board',sessionId:M.state?.settings?.selectedSessionId||'',...extra});
  ...
  function go(view,{push=true,restore=true,restoreScroll:restoreOpt}={}){
    if(!views.has(view))view='board';
    M.boardStateEngine?.cancelWork?.();
    rememberScroll();
    closeTransient();
    M.state.settings=M.state.settings||{};
    M.state.settings.view=view;
    active(view);
    if(push){try{history.pushState(state(view),'',`#${view}`)}catch{}}
    renderView(view);
    renderExtra(view);
    active(view);
    saveUi();
