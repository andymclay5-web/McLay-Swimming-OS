...
(function(g){
  ...
  document.addEventListener('click',e=>{
    ...
    const ath=e.target.closest?.('[data-msos-ath]');if(ath){e.preventDefault();e.stopImmediatePropagation();M.state.settings.selectedAthleteId=ath.dataset.msosAth;M.state.settings.selectedSwimmerId=ath.dataset.msosAth;saveUi();V.go((M.access?.role?.()||'owner')==='swimmer'?'swimmer':'athletes',{restore:false});return;}
  },true);
