...
(function(g){
  ...
  const N=M.nav,UI=M.ui,V=M.navigationEngine={build:'v4-navigation-20260825-guardian-direct'};
  ...
  const renderView=view=>{
    if(view==='guardian'){
      const host=document.querySelector('#guardianView');
      if(host)host.innerHTML='<section class="page-card"><div class="eyebrow">GUARDIAN</div><h1>Running device checks…</h1><p>Checking this phone without running the full regression suite on deck.</p></section>';
      if(typeof M.guardian?.runAndRender==='function')requestAnimationFrame(()=>M.guardian.runAndRender());
      else UI.renderGuardian?.({ok:false,passed:0,total:1,tests:[{name:'Guardian runtime available',ok:false,detail:'Guardian runtime not loaded'}],build:M.BUILD,contract:'device-health-runtime-missing'});
      return;
    }
    UI.renderCurrent?.();
  };
  N.rememberScroll=rememberScroll;N.restoreScroll=restoreScroll;N.clearTransient=closeTransient;
  V.go=(view,{push=true,restore=true,restoreScroll:restoreOpt}={})=>{if(!views.has(view))view='board';M.boardStateEngine?.cancelWork?.();rememberScroll();closeTransient();M.state.settings=M.state.settings||{};M.state.settings.view=view;active(view);saveUi();if(push){try{history.pushState(N.state?.(view)||{msos:true,msosView:view,sessionId:M.state.settings.selectedSessionId||''},'',`#${view}`)}catch{}}renderView(view);renderExtra(view);active(view);const doRestore=restoreOpt===undefined?restore:restoreOpt;if(doRestore)restoreScroll(view);else requestAnimationFrame(()=>window.scrollTo(0,0));};
  N.rememberScroll=rememberScroll;N.restoreScroll=restoreScroll;N.clearTransient=closeTransient;
  N.show=V.go;N.activateView=active;
  N.dismissLayer=()=>{const layer=history.state?.layer;if(layer){closeTransient();saveUi();history.back();return true;}closeTransient();saveUi();M.boardStateEngine?.cancelWork?.();renderView(M.state?.settings?.view||'board');renderExtra(M.state?.settings?.view||'board');return false;};
  N.applyHistory=state=>{M.boardStateEngine?.cancelWork?.();const view=views.has(state?.msosView)?state.msosView:'board';M.state.settings=M.state.settings||{};M.state.settings.view=view;if(state?.sessionId&&M.state.canonicalSessions?.[state.sessionId])M.state.settings.selectedSessionId=state.sessionId;if(!state?.layer)closeTransient();else if(state.layer.type==='item')M.state.settings.expandedItemId=state.layer.id;active(view);saveUi();renderView(view);renderExtra(view);restoreScroll(view);};
