...
(function(g){
  ...
  function payloadFor(a,onStep){
    ...
    step('sessions');const sessions=sessionsFor(a),currentId=String(M.currentSession?.()?.id||''),session=sessions.find(s=>s.id===currentId)||safeSession(a);
    ...
    return{schema:'msos-swimmer-portal-v6',publishedAt:new Date().toISOString(),athlete:{id:String(a.id),full_name:a.full_name||'',preferred_name:a.preferred_name||a.nickname||'',squad:a.squad||'',date_of_birth:a.date_of_birth||'',current_s_class:a.current_s_class||'',current_sb_class:a.current_sb_class||'',current_sm_class:a.current_sm_class||''},session,sessions,performance,training,tests,meet,sharedEvidence};
  }
  ...
  X.payloadFor=payloadFor;X.safeSession=safeSession;X.sessionsFor=sessionsFor;X.safePerformance=safePerformance;X.safeTests=safeTests;X.safeMeet=safeMeet;X.sessionActionsFor=sessionActionsFor;X.verifySessionInteractionLayer=verifySessionInteractionLayer;X.acknowledgeSessionAction=acknowledgeSessionAction;X.rpc=rpc;X.installButton=installButton;
})(globalThis);
