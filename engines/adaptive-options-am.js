...
(function(g){
  ...
  const A=M.adaptiveDelivery={build:'v4-adaptive-calendar-20260821am'};
  ...
  const activeOverride=(session,item,ath)=>(M.state?.adaptationOverrides||[]).find(x=>x.sessionId===session?.id&&x.itemId===item?.id&&x.athleteId===ath?.id&&x.active!==false)||null;
  ...
  function amberMode(out,item,ath,state,session){
    ...
    const ov=activeOverride(session,item,ath),requested=text(ov?.patch?.adaptiveMode),available=AMBER_MODES.map(x=>x.id),fallback=available[deterministic(item,available.length)],mode=available.includes(requested)?requested:fallback,stroke=E.Evidence?.stroke?.(ov?.patch?.stroke||'Choice')||'Choice',meta=AMBER_MODES.find(x=>x.id===mode)||AMBER_MODES[0];
  ...
  function conorMode(out,item,ath,state,session){
    ...
    const ov=activeOverride(session,item,ath),requested=text(ov?.patch?.adaptiveMode),available=CONOR_MODES.map(x=>x.id),mode=available.includes(requested)?requested:'Choice non-Breaststroke';
  ...
  const adapt=(item,ath,state=M.state,session=current())=>{let out=priorAdapt(item,ath,state,session);if(!out||item?.kind!=='set')return out;const k=key(ath);if(k==='amberproudfoot')out=amberMode(out,item,ath,state,session);else if(k==='conorfischer')out=conorMode(out,item,ath,state,session);return out;};
