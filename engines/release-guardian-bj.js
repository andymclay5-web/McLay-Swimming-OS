'use strict';
(function(g){
  const M=g.MSOS4,G=M?.guardian,D=M?.guardianDeviceStateBJ;if(!M||!G?.run)return;
  const BUILD='v4-guardian-full-gate-20260822bj',baseRun=G.run.bind(G);
  ...
  G.run=()=>{
    const base=baseRun()||{},device=D?.scan?.()||{ok:true,passed:0,total:0,tests:[]},tests=[...(base.tests||[]),...(device.tests||[]).map(t=>({name:`Device · ${t.name}`,ok:t.ok===true,detail:t.detail||''}))],passed=tests.filter(t=>t.ok===true).length;
