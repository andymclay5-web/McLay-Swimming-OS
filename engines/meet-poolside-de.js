...
(function(g){
  ...
  let observer=null,repairing=false,maintainQueued=false,programmeRenderQueued=false,explicitOpen=null,transitionUntil=0;
  ...
  function saveOnce(){try{M.store?.save?.(M.state)}catch{}try{M.storageEngine?.saveUi?.(M.state)}catch{}}
  function importForMeet(id){return(M.state.meetImports||[]).filter(x=>x?.meet_id===id&&x?.text).at(-1)||null}
  function parsedDeckForImport(id,imp){
    if(!imp?.text||!M.meetFieldPatch?.parseHytekProgramme)return null;
    try{const parsed=M.meetFieldPatch.parseHytekProgramme(imp.text);if(!parsed?.races?.length)return null;return{source_id:imp.id,meet_id:id,title:parsed.title,session:parsed.session,date_range:parsed.date_range,races:parsed.races,swimmers:parsed.swimmers,created_at:now()}}catch{return null}
  }
  function repairManagedDecks(){
    if(repairing)return false;repairing=true;let changed=false;
    try{
      const p=programmeState();
      for(const [id,ws] of Object.entries(p.meetWorkspaces||{})){
        if(ws?.deck?.races?.length)continue;
        const imp=importForMeet(id),deck=parsedDeckForImport(id,imp);if(!deck)continue;
        ws.deck=clone(deck);ws.program=ws.program||{};if(!Array.isArray(ws.program.sources))ws.program.sources=[];
      ...
      if(changed)saveOnce();return changed;
    }finally{repairing=false}
  }
  function repairSiscSources(){
    const p=programmeState();let invalid=false;
    for(const src of p.sources||[]){if(!/AQGCB/i.test(String(src?.raw||'')))continue;const aq=(src?.parsed?.heats||[]).flatMap(h=>h.rows||[]).filter(r=>r?.is_aquagym).length;if(!aq){delete src._sisc_format_build;delete src._sisc_raw_sig;invalid=true}}
    if(!invalid)return false;try{M.meetSiscFormat?.repair?.()}catch{}return true;
  }
  function pinMeetSwitcher(){const h=document.querySelector('#meetView'),box=h?.querySelector('[data-meet-workspace-cy]');if(!h||!box)return;if(h.firstElementChild!==box)h.insertBefore(box,h.firstElementChild);box.classList.add('meet-poolside-top')}
  function renderProgrammeSoon(force=false){
    if(programmeRenderQueued)return;programmeRenderQueued=true;
    requestAnimationFrame(()=>{programmeRenderQueued=false;if(M.state?.settings?.view!=='meet'||Date.now()<transitionUntil||(!force&&!currentDeckOwned()))return;if(hasSiscSource())try{M.meetSiscFormat?.repair?.()}catch{}try{M.meetProgramBA?.render?.()}catch{}pinMeetSwitcher();enhanceVideo();suppressLegacyWhenProgramme()});
  }
  function ensureProgrammeVisible(){const h=document.querySelector('#meetView');if(!h||M.state?.settings?.view!=='meet'||Date.now()<transitionUntil||!currentDeckOwned())return;if(h.querySelector('[data-meet-program-ba]'))return;renderProgrammeSoon(false)}
  ...
  function findOpenForAthlete(id){
    ...
    for(const src of p.sources||[]){if(!/AQGCB/i.test(String(src?.raw||'')))continue;for(const h of src?.parsed?.heats||[]){for(const row of h.rows||[]){if(!row?.is_aquagym)continue;const r=rs.find(x=>!x.relay&&x.event_number===h.event_number&&(x.heat||0)===(h.heat||0)&&(x.lane||0)===(row.lane||0)&&x.athlete_id===id);if(!r)continue;return{meetId:M.state?.settings?.currentMeetId||'',athleteId:id,expandedKey:[h.session_id,h.event_number,h.heat,row.lane||0,norm(row.name)].join('|'),selectedKey:[deck?.source_id||'field',r.event_number||0,r.heat||0,r.lane||0,r.athlete_id||r.athlete_name||''].join('|'),selectedSourceId:sourceId(src),selectedEventNumber:Number(h.event_number)||0}}}
    }return null;
  }
  function applyExplicitOpen(){if(!explicitOpenActive())return false;const p=programmeState();let changed=false;for(const [k,v] of [['selectedAthleteId',explicitOpen.athleteId],['expandedKey',explicitOpen.expandedKey],['selectedKey',explicitOpen.selectedKey],['selectedSourceId',explicitOpen.selectedSourceId]])if(v&&p[k]!==v){p[k]=v;changed=true}if(explicitOpen.selectedEventNumber&&Number(p.selectedEventNumber)!==explicitOpen.selectedEventNumber){p.selectedEventNumber=explicitOpen.selectedEventNumber;changed=true}return changed}
  function openAthlete(id){if(!id||!hasSiscSource())return;repairSiscSources();const lock=findOpenForAthlete(id);if(!lock)return;explicitOpen=lock;applyExplicitOpen();saveOnce();renderProgrammeSoon(true)}
  function raceForCaptureButton(btn){const k=btn?.dataset?.baCapture||'';return(M.state?.meetFieldDeck?.races||[]).find(r=>M.meetOpsEngine?.keyFor?.(r)===k)||null}
  ...
  function style(){
    if(document.getElementById('meet-poolside-de-style'))return;const s=document.createElement('style');s.id='meet-poolside-de-style';s.textContent=`
      ...
      body.meet-program-authority-active #meetView [data-meet-ops-av],body.meet-program-authority-active #meetView [data-meet-board-ay],body.meet-program-authority-active #meetView [data-meet-board-az],body.meet-program-authority-active #meetView [data-meet-field-deck-au]{display:none!important}
  ...
  function install(){
    ...
    document.addEventListener('click',e=>{if(e.target?.closest?.('[data-mwm-create]'))beginTransition();const b=e.target?.closest?.('[data-mwm-meet]');if(b&&b.dataset.mwmMeet!==(M.state?.settings?.currentMeetId||''))beginTransition()},true);
    document.addEventListener('click',e=>{
      ...
      const row=e.target?.closest?.('[data-ba-row].aqua');if(row&&hasSiscSource()&&!e.target?.closest?.('button,details,summary,input,textarea'))queueMicrotask(()=>{const p=programmeState();if(!p.expandedKey){explicitOpen=null;return}explicitOpen={meetId:M.state?.settings?.currentMeetId||'',athleteId:p.selectedAthleteId||'',expandedKey:p.expandedKey,selectedKey:p.selectedKey||'',selectedSourceId:p.selectedSourceId||'',selectedEventNumber:Number(p.selectedEventNumber)||0}});
      if(!e.target?.closest?.('[data-mfa-use]'))return;setTimeout(()=>{try{M.meetWorkspaceEngine?.snapshotCurrent?.({persist:true})}catch{}repairManagedDecks();queueMaintain()},80);
    },false);
    const h=document.querySelector('#meetView');if(h&&!observer){observer=new MutationObserver(queueMaintain);observer.observe(h,{childList:true,subtree:false})}
  }
