...
(function(g){const M=g.MSOS4,E=g.MSOSEngines;if(!M||!E?.Evidence||!E?.Aerobic||!E?.RacePace||!E?.Modification||!E?.Coordinator)throw new Error('MSOS engine bridge dependencies missing');
  ...
  B.ensurePathwayAthlete=(ath,course='SCM')=>{if(!ath||!M.pathway?.profile)return 0;let profile=null;try{profile=M.pathway.profile(ath,course)}catch{return 0}const stable=M.util?.stableId||((p,...v)=>`${p}-${v.join('-')}`),rows=[];for(const ev of profile?.events||[]){const pb=ev?.pb;if(!pb)continue;const seconds=E.Evidence.seconds(pb),distance=E.Evidence.distance(pb)||Number(ev?.distance||ev?.event_distance),stroke=E.Evidence.rowStroke(pb)||E.Evidence.stroke(ev?.stroke||ev?.event_stroke||'');if(!Number.isFinite(seconds)||seconds<=0||!distance||!stroke)continue;const points=Number(ev?.points?.value??pb?.wa_points??pb?.world_aquatics_points??pb?.para_points??pb?.points);rows.push({...pb,id:pb.id||stable('pathway-pb',ath.id,course,distance,stroke,seconds),athlete_id:ath.id,distance,stroke,pool_course:E.Evidence.course(pb)||course,result_seconds:seconds,wa_points:Number.isFinite(points)&&points>0?points:pb.wa_points,source_type:pb.source_type||'pathway',source_label:pb.source_label||'Swimmer pathway evidence'});}if(rows.length){M.state.resultsPbBoard=E.Evidence.merge(M.state.resultsPbBoard||[],rows);g.MSOSEvidenceIndex?.invalidate?.(M.state);E.Coordinator.clearCache?.();}return rows.length;};
  B.hydrate=async({force=false}={})=>{if(B.hydrated&&!force)return true;if(B.hydrating&&!force)return B.hydrating;B.hydrating=(async()=>{try{
      ...
      // Hydration is background reference work. It must never own the active Board render or navigation.
      ...
      let refs={pb:0,history:0,coach:0,base:0};try{await M.refs?.boot?.();refs=mergeReferenceEvidence();}catch{}
      E.Evidence.ensureVerified?.(M.state);M.state._evidenceBridge={...refs,hydratedAt:new Date().toISOString()};g.MSOSEvidenceIndex?.invalidate?.(M.state);E.Coordinator.clearCache?.();B.hydrated=true;
      const v=M.state?.settings?.view||'board';if(['athletes','reports','times','swimmer'].includes(v))requestAnimationFrame(()=>M.ui?.renderCurrent?.());
      return true;
