...
(function(g){const M=g.MSOS4,E=g.MSOSEngines;if(!M||!E?.Evidence||!E?.Aerobic||!E?.RacePace||!E?.Modification||!E?.Coordinator)throw new Error('MSOS engine bridge dependencies missing');
  const B={build:'v4-engine-silos-20260820o',hydrated:false,hydrating:null};M.engineBridge=B;M.BUILD=B.build;M.CORE='20260820-engine-silos-o';
  ...
  B.hydrate=async({force=false}={})=>{if(B.hydrated&&!force)return true;if(B.hydrating&&!force)return B.hydrating;B.hydrating=(async()=>{try{const snap=await E.Evidence.hydrate(M.state||{});for(const k of ['athletes','trainingTestTypes','trainingTestResults','adaptationProfiles','adaptationOverrides','coachResults','resultsEventHistory','resultsPbBoard','courseConversions','worldAquaticsBaseTimes'])if(Array.isArray(snap[k]))M.state[k]=E.Evidence.merge(M.state[k],snap[k]);M.state._refs=snap._refs||M.state._refs||{};E.Evidence.ensureVerified?.(M.state);const pathway=pathwayEvidence();if(pathway.length)M.state.resultsPbBoard=E.Evidence.merge(M.state.resultsPbBoard||M.state.results_pb_board||[],pathway);M.state._evidenceBridge={pathwayRows:pathway.length,hydratedAt:new Date().toISOString()};E.Coordinator.clearCache?.();B.hydrated=true;M.ui?.renderCurrent?.();return true;}catch(err){console.warn('[MSOS] evidence hydration failed',err);return false;}finally{B.hydrating=null;}})();return B.hydrating;};
