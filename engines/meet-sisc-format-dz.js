...
(function(g){
  ...
  const BUILD='v4-meet-sisc-format-20260828dz5-lifecycle';
  ...
  function parse(raw,id='session'){
    const lines=String(raw||'').replace(/\r/g,'').split('\n'),events=[];let title='',session='',date='',ev=null,heat=null;
    const addHeat=(n,start='')=>{let h=ev?.heats.find(x=>x.heat===n);if(ev&&!h){h={heat:n,start_time:txt(start),rows:[]};ev.heats.push(h)}return h};
    for(const rawLine of lines){const line=rawLine.trim();if(!line)continue;
      ...
      const em=line.match(/^Event\s+(\d+)\s+(.+)$/i);if(em){ev={event_number:Number(em[1]),event:txt(em[2]),...info(em[2]),heats:[]};events.push(ev);heat=null;continue}
      const hm=line.match(/^Heat\s+(\d+)(?:\s+of\s+\d+)?(?:\s+(?:Finals?|Prelims?|Heats?|Semi(?:-?Finals?)?|Timed Finals?))?(?:\s+\(#\d+[^)]*\))?(?:\s+Starts at\s+(.+))?/i);if(hm){heat=addHeat(Number(hm[1]),hm[2]||'');continue}
      ...
      if(ev.relay){const rm=rest.match(/^(.+?)\s+([A-Z])$/i);if(rm){const club=txt(rm[1]);heat.rows.push({kind:'relay',lane,team:club,team_code:rm[2],club,seed_time:seed,seed_seconds:sec(seed),legs:[],is_aquagym:isAQ(club)})}continue}
      ...
      if(m){const club=txt(m[3]);const sex=/^Women|Girls/i.test(ev.event)?'W':/^Men|Boys/i.test(ev.event)?'M':'';heat.rows.push({kind:'individual',lane,name:txt(m[1]),classification:'',sex,age:Number(m[2]),club,seed_time:seed,seed_seconds:sec(seed),is_aquagym:isAQ(club)})}
    }
