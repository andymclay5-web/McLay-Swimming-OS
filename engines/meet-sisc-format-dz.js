...
(function(g){
  ...
  const BUILD='v4-meet-sisc-format-20260827dz';
  ...
  function repair(){const s=M.state?.meetProgramBA;if(!s?.sources?.length)return false;let changed=false;for(const src of s.sources){if(!src?.raw||!/AQGCB/i.test(src.raw))continue;const p=parse(src.raw,src.source_id||'session');const aq=p.heats.flatMap(h=>h.rows).filter(r=>r.is_aquagym).length;if(aq){src.parsed=p;changed=true}}if(changed){try{M.store?.save?.(M.state)}catch{}try{M.storageEngine?.saveUi?.(M.state)}catch{}}return changed}
  const baseRender=M.meetProgramBA.render?.bind(M.meetProgramBA);if(baseRender)M.meetProgramBA.render=()=>{repair();return baseRender()};
  const observer=new MutationObserver(()=>{if(M.state?.settings?.view==='meet'&&repair())queueMicrotask(()=>baseRender?.())});
  const h=document.querySelector('#meetView');if(h)observer.observe(h,{childList:true,subtree:false});
  setTimeout(()=>{if(repair())baseRender?.()},0);
